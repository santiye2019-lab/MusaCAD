...
The bridge in app/src/main/cpp/dwg_bridge.c reads a DWG and writes ASCII DXF
using the same API sequence as LibreDWG's programs/dwg2dxf.c. The same bridge
is also built as a host executable for smoke tests. Test input is the pinned
upstream example_2013.dwg. The JNI library still requires Android device tests.
...
Runtime limits: native conversion itself is not interruptible. Cancellation
discards the result after the native call returns. Rendering remains limited
to supported 2D DXF entities; conversion does not imply complete DWG fidelity.
Critical conversion failure falls back to an embedded preview only when one
exists; the screen labels that case explicitly. Original sharing sends DWG.
