...
old='''        skipped(read("",dimension("*MISSING")));\n        System.out.println("17 block expansion cases passed");\n'''
new='''        skipped(read("",dimension("*MISSING")));\n\n        String paperText=tags(0,"SECTION",2,"ENTITIES",0,"LINE",67,1,330,"7",10,3,20,4,11,5,21,4,\n            0,"ENDSEC",0,"SECTION",2,"OBJECTS",0,"LAYOUT",100,"AcDbPlotSettings",1,"",330,"ROOT",100,"AcDbLayout",1,"A4",330,"7",\n            0,"ENDSEC",0,"EOF");\n        DxfBlocks.Result paper=DxfBlocks.expand(Arrays.asList(paperText.split("\\n")),"A4");\n        if(!"A4".equals(paper.activeLayout)||paper.placements.size()!=1||!paper.layouts.contains("A4"))throw new AssertionError("Layout owner resolution");\n        System.out.println("18 block expansion/layout cases passed");\n'''
if s.count(old)!=1: raise SystemExit('DxfBlocksTest marker mismatch')
