...
/** Pure-Java parser for supported 2D paper-space VIEWPORT entities. */
public final class DxfViewport {
    ...
    public static final class View {
        ...
        public final String clipBoundaryHandle;
        View(double pcx,double pcy,double pw,double ph,double vcx,double vcy,double vh,double twist,double tx,double ty,double dx,double dy,double dz,int status,int id,int flags,String clipBoundaryHandle){
            paperCenterX=pcx;paperCenterY=pcy;paperWidth=Math.abs(pw);paperHeight=Math.abs(ph);viewCenterX=vcx;viewCenterY=vcy;viewHeight=Math.abs(vh);twistDegrees=twist;targetX=tx;targetY=ty;directionX=dx;directionY=dy;directionZ=dz;this.status=status;this.id=id;this.flags=flags;this.clipBoundaryHandle=clipBoundaryHandle==null?"":clipBoundaryHandle.trim();
        }
        /** Only rectangular top-view 2D viewports are rendered until group-340 clip geometry is resolved. */
        public boolean active2d(){return id>1&&status>0&&(flags&FLAG_OFF)==0&&(flags&FLAG_PERSPECTIVE)==0&&rectangular()&&paperWidth>0d&&paperHeight>0d&&viewHeight>0d&&Math.abs(directionX)<1e-7&&Math.abs(directionY)<1e-7&&directionZ>0d;}
        public boolean rectangular(){return (flags&FLAG_NON_RECTANGULAR)==0;}
        public boolean hasExternalClipBoundary(){return !rectangular()&&!clipBoundaryHandle.isEmpty();}
    ...
    public static View parse(List<String>a,int from,int to){
        return new View(number(a,from,to,10,0d),number(a,from,to,20,0d),number(a,from,to,40,0d),number(a,from,to,41,0d),
            ...
            integer(a,from,to,68,0),integer(a,from,to,69,0),integer(a,from,to,90,0),text(a,from,to,340,""));
    }
    ...
    private static String text(List<String>a,int from,int to,int wanted,String fallback){for(int i=from;i+1<to;i+=2)if(code(a.get(i))==wanted)return a.get(i+1)==null?fallback:a.get(i+1).trim();return fallback;}
    private DxfViewport(){}
}
