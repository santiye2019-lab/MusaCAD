...
/** Verifier for server-issued, device-bound MusaCAD one-day trial tokens. */
public final class TrialToken {
    public static final String PREFIX="MT1";
    ...
    public static Result verify(String token,String expectedInstallationId,long nowMs,String publicKeyPem){
        try{
            ...
            if(nowMs>expiresAt)return fail("expired");
    ...
    private static PublicKey parsePublicKey(String pem)throws Exception{
        if(pem==null||pem.trim().isEmpty())throw new IllegalArgumentException("publicKey");
        String b64=pem.replace("-----BEGIN PUBLIC KEY-----","").replace("-----END PUBLIC KEY-----","").replaceAll("\\s","");
        return KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(decode64(b64,false)));
    }
    ...
    private static byte[] decode64(String s,boolean url)throws Exception{
        ...
        int mod=clean.length()&3;if(mod==1)throw new IllegalArgumentException("base64 length");
        ...
        for(int i=0;i<clean.length();i++){
            ...
            acc=(acc<<6)|v;bits+=6;if(bits>=8){bits-=8;out.write((acc>>>bits)&255);}
        }
