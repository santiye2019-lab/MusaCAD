...
public final class TrialService {
    ...
    public static Result start(Context context){
        ...
        String deviceId=LicenseManager.installationId(context);
        if(!DeviceIdentityHash.isValidPublicId(deviceId))return new Result(Status.DENIED,"Bu telefonda kalıcı MusaCAD cihaz kimliği alınamadı");
        ...
        if(endpoint.isEmpty()||trialKey.isEmpty())return new Result(Status.NOT_CONFIGURED,"Deneme sunucusu veya doğrulama anahtarı yapılandırılmadı");
        if(!endpoint.startsWith("https://"))return new Result(Status.NOT_CONFIGURED,"Deneme sunucusu adresi HTTPS olmalıdır");
        ...
        try{
            ...
            JSONObject request=new JSONObject();request.put("deviceId",deviceId);request.put("packageName",context.getPackageName());request.put("versionName",BuildConfig.VERSION_NAME);request.put("versionCode",BuildConfig.VERSION_CODE);
            ...
            if(code>=300&&code<400)return new Result(Status.DENIED,"Deneme sunucusu yönlendirme döndürdü");
            ...
            if(response.isEmpty())return new Result(Status.INVALID_RESPONSE,"Boş deneme sunucusu yanıtı");
            ...
            if("active".equals(status)){
                ...
                if(LicenseManager.activateTrialToken(context,token))return new Result(Status.ACTIVATED,"Deneme etkinleştirildi");
                return new Result(Status.INVALID_RESPONSE,"Deneme imzası veya süresi geçersiz");
            }
            ...
            if("denied".equals(status)||code==401||code==403)return new Result(Status.DENIED,json.optString("message","Deneme isteği reddedildi"));
            return new Result(Status.INVALID_RESPONSE,json.optString("message","Tanımsız deneme sunucusu yanıtı"));
        }catch(IOException e){return new Result(Status.NETWORK_ERROR,"Deneme başlatmak için internet bağlantısını kontrol edin");}
        catch(Exception e){return new Result(Status.INVALID_RESPONSE,"Deneme sunucusu yanıtı doğrulanamadı");}
