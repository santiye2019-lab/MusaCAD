...
public final class DxfTextStyle {
    ...
    public static Map<String,Style> parse(List<String> tags){
        ...
        for(int i=0;i+1<tags.size();){
            ...
            String name=text(tags,from,to,2,STANDARD);Style style=new Style(name,text(tags,from,to,3,""),text(tags,from,to,4,""),number(tags,from,to,40,0d),number(tags,from,to,41,1d),number(tags,from,to,50,0d),integer(tags,from,to,70,0),integer(tags,from,to,71,0));out.put(style.name,style);i=to;
        }
        return defaults(out);
    }
