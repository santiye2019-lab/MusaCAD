...
public final class DxfTextStyle {
    ...
    public static final class Style {
        ...
        private final boolean fontFamilyName;
        ...
        Style(String name,String fontFile,String bigFontFile,double fixedHeight,double widthFactor,double obliqueDegrees,int flags,int generationFlags){
            this(name,fontFile,bigFontFile,fixedHeight,widthFactor,obliqueDegrees,flags,generationFlags,false);
        }

        Style(String name,String fontFile,String bigFontFile,double fixedHeight,double widthFactor,double obliqueDegrees,int flags,int generationFlags,boolean fontFamilyName){
        ...
        public boolean usesShx(){
            if(fontFamilyName)return false;
            String f=fontFile.toLowerCase(Locale.ROOT);
            return f.endsWith(".shx")||(!f.isEmpty()&&!f.contains("."));
        }

        public String familyHint(){
            if(usesShx())return "monospace";
            String f=fontFile.replace('\\','/');
    ...
    public static Map<String,Style> parse(List<String> tags){
        LinkedHashMap<String,Style> out=new LinkedHashMap<>();
        if(tags==null)return defaults(out);
        for(int i=0;i+1<tags.size();){
            if(code(tags.get(i))!=0||!"STYLE".equalsIgnoreCase(tags.get(i+1).trim())){i+=2;continue;}
            int from=i+2,to=from;
            while(to+1<tags.size()&&code(tags.get(to))!=0)to+=2;

            String name=text(tags,from,to,2,STANDARD);
            String font=text(tags,from,to,3,"");
            boolean familyName=false;
            if(font.isEmpty()){
                font=extendedFontHint(tags,from,to);
                familyName=!font.isEmpty()&&!looksLikeFontFile(font);
            }
            ...
            Style style=new Style(
                ...
                font,
                text(tags,from,to,4,""),
                number(tags,from,to,40,0d),
                number(tags,from,to,41,1d),
                number(tags,from,to,50,0d),
                integer(tags,from,to,70,0),
                integer(tags,from,to,71,0),
                familyName
            );
            out.put(style.name,style);
            i=to;
        }
        return defaults(out);
    }
    ...
    public static Style resolve(Map<String,Style> styles,String name){
        if(styles!=null){
            Style s=styles.get(normalize(name));
            if(s!=null)return s;
            s=styles.get(STANDARD);
            if(s!=null)return s;
        }
        return defaultStyle();
    }
    ...
    private static Map<String,Style> defaults(LinkedHashMap<String,Style> out){
        if(!out.containsKey(STANDARD))out.put(STANDARD,defaultStyle());
    ...
    private static String extendedFontHint(List<String>a,int from,int to){
        for(int i=from;i+1<to;i+=2){
            if(code(a.get(i))==1000){
                String v=a.get(i+1).trim();
                if(!v.isEmpty()&&v.length()<=128)return v;
            }
        }
        return "";
    }

    private static boolean looksLikeFontFile(String value){
        String v=clean(value).replace('\\','/');
        int slash=v.lastIndexOf('/');
        if(slash>=0)v=v.substring(slash+1);
        int dot=v.lastIndexOf('.');
        return dot>0&&dot<v.length()-1;
    }

    private static String text(List<String>a,int from,int to,int wanted,String fallback){
        for(int i=from;i+1<to;i+=2)if(code(a.get(i))==wanted)return a.get(i+1).trim();
        return fallback;
    }

    private static double number(List<String>a,int from,int to,int wanted,double fallback){
        try{return Double.parseDouble(text(a,from,to,wanted,Double.toString(fallback)));}catch(Exception e){return fallback;}
    }

    private static int integer(List<String>a,int from,int to,int wanted,int fallback){
        try{return Integer.parseInt(text(a,from,to,wanted,Integer.toString(fallback)));}catch(Exception e){return fallback;}
    }
