...
public final class DxfBlocks {
    public static final class Transform {
        ...
        static Transform insert(double bx,double by,double sx,double sy,double degrees,double x,double y){
            double r=Math.toRadians(degrees),co=Math.cos(r),si=Math.sin(r);double a=co*sx,b=si*sx,c=-si*sy,d=co*sy;
            return new Transform(a,b,c,d,x-a*bx-c*by,y-b*bx-d*by);
        }
    }

    public static final class Record {
        public final String type;public final int from,to;private final List<String> tags;
    ...
    public static final class Placement {
        public final Record record;public final Transform transform;public final String layer;public final int color;public final String lineType;public final double lineTypeScale;public final int lineWeight;public final double blockScale;public final boolean directRoot;
        Placement(Record record,Transform transform,String layer,int color,String lineType,double lineTypeScale,int lineWeight,boolean directRoot){this.record=record;this.transform=transform;this.layer=layer;this.color=color;this.lineType=DxfLineStyle.normalizeName(lineType);this.lineTypeScale=lineTypeScale;this.lineWeight=lineWeight;this.blockScale=transform.scale();this.directRoot=directRoot;}
    }

    private static final class Block {final Record header;final List<Record> members=new ArrayList<>();Block(Record header){this.header=header;}}

    public static final class Result {
        public final List<Placement> placements=new ArrayList<>();public final Map<String,Integer> layerColors=new LinkedHashMap<>();public final Map<String,String> layerLineTypes=new LinkedHashMap<>();public final Map<String,Integer> layerLineWeights=new LinkedHashMap<>();public final Map<String,DxfLineStyle.Pattern> lineTypes=new LinkedHashMap<>();public int skipped;private int visits;
    }
    ...
    public static Result expand(List<String> tags,int defaultLineweight)throws IOException{
        ...
        try{
            if(tags.size()%2!=0)throw new NumberFormatException();for(int i=0;i<tags.size();i+=2)Integer.parseInt(tags.get(i).trim());
            for(int i=0;i<tags.size();){if(Integer.parseInt(tags.get(i).trim())!=0){i+=2;continue;}String type=tags.get(i+1).trim();int from=i+2;i=from;while(i<tags.size()&&Integer.parseInt(tags.get(i).trim())!=0)i+=2;records.add(new Record(type,tags,from,i));}
        ...
        for(Record record:records){
            ...
            if(section.equals("TABLES")&&record.type.equals("LTYPE")){String name=DxfLineStyle.normalizeName(record.text(2,DxfLineStyle.CONTINUOUS));result.lineTypes.put(name,new DxfLineStyle.Pattern(name,record.numbers(49),record.has(74)||record.has(75)));}
        ...
        int defaultBlockColor=DxfColor.aciArgb(DxfColor.DEFAULT_ACI);for(Record root:roots)expand(root,new Transform(),"0",defaultBlockColor,DxfLineStyle.CONTINUOUS,defaultWeight,1d,true,blocks,layerColorLookup,layerLineTypeLookup,layerLineWeightLookup,defaultWeight,new HashSet<>(),result);return result;
    }

    private static void expand(Record r,Transform parent,String parentLayer,int parentBlockColor,String parentBlockLineType,int parentBlockLineWeight,double parentLineTypeScale,boolean directRoot,Map<String,Block>blocks,Map<String,Integer>layerColors,Map<String,String>layerLineTypes,Map<String,Integer>layerLineWeights,int defaultLineweight,Set<String>stack,Result result)throws IOException{
        if(Thread.currentThread().isInterrupted())throw new java.io.InterruptedIOException("Yükleme iptal edildi");if(++result.visits>100000)throw new IOException("DXF blokları açıldığında nesne sınırı aşıldı");if(r.type.equals("SEQEND"))return;
        String layer=r.text(8,"0");if(layer.equals("0"))layer=parentLayer;int color=entityColor(r,layer,parentBlockColor,layerColors);String lineType=entityLineType(r,layer,parentBlockLineType,layerLineTypes);int lineWeight=entityLineweight(r,layer,parentBlockLineWeight,layerLineWeights,defaultLineweight);double ownScale=r.number(48,1d);if(!Double.isFinite(ownScale)||ownScale<=0d)ownScale=1d;double effectiveLineTypeScale=parentLineTypeScale*ownScale;

        // DIMENSION references an anonymous block containing its rendered picture. For ordinary 2D WCS dimensions, group 12/22 translates that picture block.
        if(r.type.equals("DIMENSION")){
            ...
            if(block!=null&&!stack.contains(name)&&stack.size()<32&&defaultExtrusion){
                Transform transform=parent.thenLocal(Transform.translate(r.number(12,0),r.number(22,0)));stack.add(name);
                for(Record member:block.members)expand(member,transform,layer,color,lineType,lineWeight,effectiveLineTypeScale,false,blocks,layerColors,layerLineTypes,layerLineWeights,defaultLineweight,stack,result);
                stack.remove(name);return;
            }
        }
        if(!r.type.equals("INSERT")){result.placements.add(new Placement(r,parent,layer,color,lineType,effectiveLineTypeScale,lineWeight,directRoot));return;}
        ...
        double sx=r.number(41,1),sy=r.number(42,1);if(sx==0||sy==0){result.skipped++;return;}Transform local=Transform.insert(block.header.number(10,0),block.header.number(20,0),sx,sy,r.number(50,0),r.number(10,0),r.number(20,0));Transform transform=parent.thenLocal(local);stack.add(name);for(Record member:block.members)expand(member,transform,layer,color,lineType,lineWeight,effectiveLineTypeScale,false,blocks,layerColors,layerLineTypes,layerLineWeights,defaultLineweight,stack,result);stack.remove(name);
    }
