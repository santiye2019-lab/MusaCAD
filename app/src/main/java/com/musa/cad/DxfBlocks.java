...
/** Expands ordinary 2D INSERTs and resolves DXF layer/entity colors. */
public final class DxfBlocks {
    ...
    public static Result expand(List<String> tags)throws IOException{
        ...
        for(Record record:records){
            ...
            if(section.equals("TABLES")&&record.type.equals("LAYER")){
                String name=record.text(2,"0");
                int color=layerColor(record);
                layerColorLookup.put(key(name),color);result.layerColors.put(name,color);
    ...
    private static int layerColor(Record record)throws IOException{
        ...
        int aci=Math.abs(record.integer(62,DxfColor.DEFAULT_ACI));
        return DxfColor.aciArgb(aci>=1&&aci<=255?aci:DxfColor.DEFAULT_ACI);
    }
