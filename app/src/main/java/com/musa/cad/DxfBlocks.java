...
public final class DxfBlocks {
    ...
    private static final class Block {
        ...
        File file;Charset charset;long offset;
    ...
    public interface Sink { void accept(Placement placement)throws IOException; }
    /** Index block locations first; then stream entities without retaining raw drawing text. */
    public static Result expand(File file,Charset charset,Sink sink)throws IOException {
        ...
        try(DxfStream input=new DxfStream(file,charset)){
            String section="";Record r;boolean eof=false;
            while((r=input.next())!=null){
                if(r.type.equals("SECTION"))section=r.text(2,"");
                else if(r.type.equals("ENDSEC"))section="";
                else if(r.type.equals("EOF")){eof=true;break;}
                else if(section.equals("BLOCKS")&&r.type.equals("BLOCK")){
                    if(blocks.size()>=100000)throw new IOException("DXF blok sayısı sınırı aşıldı");
                    Block block=new Block(r);block.file=file;block.charset=charset;block.offset=input.nextRecordOffset;
                    blocks.put(key(r.text(2,"")),block);
                }
            }
            if(!eof)throw new IOException("DXF dosyası tamamlanmamış (EOF yok)");
        }
        Result result=new Result();result.sink=sink;
        try(DxfStream input=new DxfStream(file,charset)){
            String section="";Record r;
            Transform identity=new Transform();Set<String> stack=new HashSet<>();
            while((r=input.next())!=null){
                if(r.type.equals("SECTION"))section=r.text(2,"");
                else if(r.type.equals("ENDSEC"))section="";
    ...
    private static void expand(Record r,Transform parent,String parentLayer,Map<String,Block> blocks,Set<String> stack,Result result)throws IOException{
        ...
        if(++result.visits>1000000)throw new IOException("DXF blokları açıldığında nesne sınırı aşıldı");
        if(r.type.equals("SEQEND"))return;
        String layer=r.text(8,"0");if(layer.equals("0"))layer=parentLayer;
        if(!r.type.equals("INSERT")){Placement placement=new Placement(r,parent,layer);if(result.sink!=null)result.sink.accept(placement);else result.placements.add(placement);return;}
        ...
        if(block.file==null){
            ...
            try(DxfStream input=new DxfStream(block.file,block.charset)){
                input.seek(block.offset);Record member;boolean ended=false;
                while((member=input.next())!=null){
                    if(member.type.equals("ENDBLK")){ended=true;break;}
                    if(member.type.equals("ENDSEC")||member.type.equals("EOF")||member.type.equals("BLOCK"))break;
                    expand(member,transform,layer,blocks,stack,result);
                }
