...
public final class SourceEditSession {
    private static final class Entry {
        final int id;final SourceRange range;final CadEdit original;final String layer;final int color;final String lineType;final double lineTypeScale;final int lineWeight;
    ...
    public boolean replaceSelectedAndOther(int otherId,SourceRange otherRange,CadEdit otherPrototype,String otherLayer,int otherColor,String otherLineType,double otherLineTypeScale,int otherLineWeight,CadEdit selectedReplacement,CadEdit otherReplacement){
        ...
        undo.addFirst(new Undo(first.id,first.copy(),otherId,previousOther==null?null:previousOther.copy(),selected));
    ...
    public boolean undo(){Undo u=undo.pollFirst();if(u==null)return false;for(int i=0;i<u.ids.length;i++){if(u.previous[i]==null)entries.remove(u.ids[i]);else entries.put(u.ids[i],u.previous[i].copy());}selected=u.selected;return true;}
