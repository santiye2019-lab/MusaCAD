...
public final class SourceEditSession {
    private static final class Entry {
        final int id;final SourceRange range;final CadEdit original;final String layer;final int color;CadEdit replacement;boolean deleted;
        Entry(int id,SourceRange range,CadEdit original,String layer,int color){this.id=id;this.range=range;this.original=original.copy();this.layer=layer==null?"0":layer;this.color=color;}
        CadEdit current(){return replacement==null?original:replacement;}
        Entry copy(){Entry e=new Entry(id,range,original,layer,color);e.replacement=replacement==null?null:replacement.copy();e.deleted=deleted;return e;}
    }
    ...
    public void select(int id,SourceRange range,CadEdit prototype){select(id,range,prototype,"0",0xFFFFFFFF);}
    public void select(int id,SourceRange range,CadEdit prototype,String layer,int color){
        if(id<0||range==null||prototype==null){selected=-1;return;}
        Entry e=entries.get(id);if(e==null){e=new Entry(id,range,prototype,layer,color);entries.put(id,e);}selected=e.deleted?-1:id;
    }
    ...
    public boolean moveSelectedTo(float x,float y){
        Entry e=entries.get(selected);if(e==null||e.deleted)return false;save(e);CadEdit c=e.current();e.replacement=c.translated(x-c.centerX(),y-c.centerY());return true;
    }

    public boolean rotateSelected(float degrees){
        Entry e=entries.get(selected);if(e==null||e.deleted)return false;save(e);CadEdit c=e.current();e.replacement=c.rotated(degrees,c.centerX(),c.centerY());return true;
    }
    ...
    public boolean undo(){
        Undo u=undo.pollFirst();if(u==null)return false;if(u.previous==null)entries.remove(u.id);else entries.put(u.id,u.previous.copy());selected=u.selected;return true;
    }
    ...
    public List<SourceReplacement> replacementRecords(){
        ArrayList<SourceReplacement> out=new ArrayList<>();for(Entry e:entries.values())if(!e.deleted&&e.replacement!=null)out.add(new SourceReplacement(e.id,e.range,e.layer,e.color,e.replacement));return out;
    }
    ...
    public int findReplacement(float x,float y,float tolerance){
        float best=Math.max(0,tolerance);int found=-1;for(Entry e:entries.values()){if(e.deleted||e.replacement==null)continue;float d=e.replacement.hitDistance(x,y);if(d<=best){best=d;found=e.id;}}return found;
    }
