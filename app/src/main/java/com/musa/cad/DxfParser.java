...
public final class DxfParser {
    ...
    private static final class Poly implements Entity{
        ...
        public void draw(Canvas c,Paint p,Matrix m){
            if(pts.size()<2)return;Path q=new Path();float[]v={pts.get(0).x,pts.get(0).y};m.mapPoints(v);q.moveTo(v[0],v[1]);
            for(int i=1;i<pts.size();i++){v[0]=pts.get(i).x;v[1]=pts.get(i).y;m.mapPoints(v);q.lineTo(v[0],v[1]);}if(closed)q.close();c.drawPath(q,p);
        }
    }
    ...
    /** Solid and patterned HATCH renderer using true DXF boundary paths. */
    private static final class HatchEntity implements Entity{
        ...
        private Path path(Matrix m){
            Path path=new Path();path.setFillType(Path.FillType.EVEN_ODD);
            for(DxfHatch.Loop loop:hatch.loops){
                if(loop.points.size()<2)continue;DxfHatch.Point first=loop.points.get(0);path.moveTo((float)first.x,(float)first.y);
                for(int i=1;i<loop.points.size();i++){DxfHatch.Point point=loop.points.get(i);path.lineTo((float)point.x,(float)point.y);}if(loop.closed)path.close();
            }
            path.transform(m);return path;
        }
        public void draw(Canvas c,Paint p,Matrix m){
            Path boundary=path(m);RectF bounds=new RectF();boundary.computeBounds(bounds,true);if(bounds.isEmpty())return;
            p.setPathEffect(null);Paint.Style old=p.getStyle();
            if(hatch.solid){p.setStyle(Paint.Style.FILL);c.drawPath(boundary,p);p.setStyle(old);return;}
            if(hatch.patternLines.isEmpty()){p.setStyle(Paint.Style.STROKE);c.drawPath(boundary,p);p.setStyle(old);return;}
            int save=c.save();c.clipPath(boundary);p.setStyle(Paint.Style.STROKE);
            double hatchAngle=Math.toRadians(hatch.patternAngleDegrees),co=Math.cos(hatchAngle),si=Math.sin(hatchAngle),scale=hatch.patternScale;
            float diag=(float)Math.hypot(bounds.width(),bounds.height())+64f;
            for(DxfHatch.PatternLine line:hatch.patternLines){
    ...
    private static final class Transformed implements Entity{
        ...
        Transformed(Entity entity,DxfBlocks.Transform t)throws IOException{
            this.entity=entity;float[]v={(float)t.a,(float)t.c,(float)t.x,(float)t.b,(float)t.d,(float)t.y,0,0,1};for(float n:v)if(!Float.isFinite(n))throw new IOException("DXF blok dönüşümü sınır dışında");matrix=new Matrix();matrix.setValues(v);
        }
    ...
    public static final class SourceEntity{
        public final int sourceId;public final SourceRange range;public final String type,layer,lineType;public final int color,lineWeight;public final double lineTypeScale;private final CadEdit prototype;
        SourceEntity(int sourceId,SourceRange range,String type,String layer,int color,String lineType,double lineTypeScale,int lineWeight,CadEdit prototype){this.sourceId=sourceId;this.range=range;this.type=type;this.layer=layer;this.color=color;this.lineType=DxfLineStyle.normalizeName(lineType);this.lineTypeScale=lineTypeScale;this.lineWeight=lineWeight;this.prototype=prototype;}
    ...
    public static final class Result{
        ...
        public Result withVisibleLayers(Set<String>selected)throws IOException{Set<String>visible=new HashSet<>(selected);visible.retainAll(layerNames);Result result=renderLayers(document,view,layerNames,visible,skippedCount,contentBounds,worldToContentScale,millimetersPerUnit,drawingUnitName,globalLineTypeScale,lineTypes);result.conversionWarnings=conversionWarnings;return result;}
        public int editableSourceCount(){return editableSources.size();}public SourceEntity sourceById(int sourceId){return sourceById.get(sourceId);}public List<SourceEntity> editableSources(){return editableSources;}
        public SourceEntity findEditableSource(float contentX,float contentY,float tolerance,Set<Integer>hiddenSourceIds){float best=Math.max(0f,tolerance);SourceEntity found=null;Set<Integer>hidden=hiddenSourceIds==null?Collections.emptySet():hiddenSourceIds;for(SourceEntity source:editableSources){if(hidden.contains(source.sourceId)||!visibleLayers.contains(source.layer))continue;float distance=source.hitDistance(contentX,contentY),allowed="TEXT".equals(source.type)?best*1.75f:best;if(distance<=allowed&&(found==null||distance<best)){best=distance;found=source;}}return found;}
        public void drawVector(Canvas canvas,Matrix imageMatrix){drawVector(canvas,imageMatrix,Collections.emptySet());}
        public void drawVector(Canvas canvas,Matrix imageMatrix,Set<Integer>hiddenSourceIds){Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG);paint.setStyle(Paint.Style.STROKE);Matrix combined=new Matrix();combined.setConcat(imageMatrix,view);Set<Integer>hidden=hiddenSourceIds==null?Collections.emptySet():hiddenSourceIds;for(Entity entity:document){LayerEntity layer=(LayerEntity)entity;if(visibleLayers.contains(layer.layer)&&!hidden.contains(layer.sourceId))layer.drawStyled(canvas,paint,combined,false,false,globalLineTypeScale);}}
        ...
        public void drawVectorForPrint(Canvas canvas,Matrix contentToPage,boolean monochrome,Set<Integer>hiddenSourceIds){Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG);paint.setStyle(Paint.Style.STROKE);Matrix combined=new Matrix();combined.setConcat(contentToPage,view);Set<Integer>hidden=hiddenSourceIds==null?Collections.emptySet():hiddenSourceIds;for(Entity entity:document){LayerEntity layer=(LayerEntity)entity;if(!visibleLayers.contains(layer.layer)||hidden.contains(layer.sourceId))continue;layer.drawStyled(canvas,paint,combined,true,monochrome,globalLineTypeScale);}}
        public void drawSourceReplacement(Canvas canvas,Matrix contentToTarget,SourceReplacement replacement,boolean print,boolean monochrome){if(canvas==null||contentToTarget==null||replacement==null||!visibleLayers.contains(replacement.layer))return;Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG);paint.setStyle(Paint.Style.STROKE);paint.setColor(monochrome?Color.BLACK:(print?paperColor(replacement.color):replacement.color));paint.setStrokeWidth(print?DxfLineStyle.printStrokePoints(replacement.lineWeight):DxfLineStyle.screenStroke(replacement.lineWeight));DxfLineStyle.Pattern pattern=lineTypes.get(DxfLineStyle.normalizeName(replacement.lineType));if(pattern==null)pattern=lineTypes.get(DxfLineStyle.CONTINUOUS);double pixelsPerDrawingUnit=matrixScale(contentToTarget)*worldToContentScale;DxfLineStyle.Dash dash=pattern==null?null:pattern.dash(pixelsPerDrawingUnit,globalLineTypeScale,replacement.lineTypeScale,1d);paint.setPathEffect(dash==null?null:new DashPathEffect(dash.intervals,dash.phase));drawContentEdit(canvas,paint,contentToTarget,replacement.edit);paint.setPathEffect(null);}
    ...
    private static final class LayerEntity implements Entity{
        final Entity entity;final String layer;final int color;final String lineType;final DxfLineStyle.Pattern linePattern;final double lineTypeScale,blockScale;final int lineWeight;final int sourceId;final SourceRange sourceRange;final String sourceType;final CadEdit sourceEditWorld;
        LayerEntity(Entity e,String l,int color,String lineType,DxfLineStyle.Pattern linePattern,double lineTypeScale,int lineWeight,double blockScale,int sourceId,SourceRange sourceRange,String sourceType,CadEdit sourceEditWorld){entity=e;layer=l;this.color=color;this.lineType=DxfLineStyle.normalizeName(lineType);this.linePattern=linePattern;this.lineTypeScale=safeLineTypeScale(lineTypeScale);this.lineWeight=lineWeight;this.blockScale=blockScale;this.sourceId=sourceId;this.sourceRange=sourceRange;this.sourceType=sourceType;this.sourceEditWorld=sourceEditWorld;}
    ...
    public static Result render(File file)throws IOException{
        ...
        if(entities.isEmpty())return null;RectF b=new RectF(Float.MAX_VALUE,Float.MAX_VALUE,-Float.MAX_VALUE,-Float.MAX_VALUE);for(Entity e:entities){FileTransfer.checkCancelled();e.bounds(b);}if(!Float.isFinite(b.left)||!Float.isFinite(b.top)||b.right<b.left||b.bottom<b.top)return null;if(b.width()==0){b.left-=.5f;b.right+=.5f;}if(b.height()==0){b.top-=.5f;b.bottom+=.5f;}float s=Math.min((SIZE-2f*MARGIN)/b.width(),(SIZE-2f*MARGIN)/b.height());Matrix m=new Matrix();m.postTranslate(-b.left,-b.bottom);m.postScale(s,-s);m.postTranslate(MARGIN+(SIZE-2*MARGIN-b.width()*s)/2f,MARGIN+(SIZE-2*MARGIN-b.height()*s)/2f);RectF contentBounds=new RectF(b);m.mapRect(contentBounds);return renderLayers(entities,m,layers,layers,skipped,contentBounds,s,millimetersPerUnit,drawingUnitName,globalLineTypeScale,expanded.lineTypes);
    }

    private static Result renderLayers(List<Entity>document,Matrix view,Set<String>all,Set<String>visible,int skipped,RectF contentBounds,float worldToContentScale,double millimetersPerUnit,String drawingUnitName,double globalLineTypeScale,Map<String,DxfLineStyle.Pattern>lineTypes)throws IOException{
        List<Entity>shown=new ArrayList<>();for(Entity entity:document){FileTransfer.checkCancelled();if(visible.contains(((LayerEntity)entity).layer))shown.add(entity);}Bitmap bitmap=Bitmap.createBitmap(SIZE,SIZE,Bitmap.Config.ARGB_8888);
        try{Canvas canvas=new Canvas(bitmap);canvas.drawColor(Color.rgb(18,24,30));Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG);paint.setStyle(Paint.Style.STROKE);for(Entity entity:shown){FileTransfer.checkCancelled();((LayerEntity)entity).drawStyled(canvas,paint,view,false,false,globalLineTypeScale);}FileTransfer.checkCancelled();return new Result(bitmap,shown.size(),skipped,snapPoints(shown,view),document,view,all,visible,contentBounds,worldToContentScale,millimetersPerUnit,drawingUnitName,globalLineTypeScale,lineTypes);}catch(IOException|RuntimeException|OutOfMemoryError e){bitmap.recycle();throw e;}
    }
