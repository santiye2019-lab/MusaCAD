...
public final class DxfParser {
    ...
    private static final class EntityGroup implements Entity{
        ...
        public void bounds(RectF b){for(Entity e:children)e.bounds(b);}
    ...
    private static final class WipeoutEntity implements Entity{
        ...
        public void draw(Canvas c,Paint p,Matrix m){drawMask(c,p,m,false);}
        void drawMask(Canvas c,Paint p,Matrix m,boolean print){
            ...
            int oldColor=p.getColor();Paint.Style oldStyle=p.getStyle();android.graphics.PathEffect oldEffect=p.getPathEffect();
            p.setPathEffect(null);p.setStyle(Paint.Style.FILL);p.setColor(print?Color.WHITE:SCREEN_BG);c.drawPath(path,p);
            p.setColor(oldColor);p.setStyle(oldStyle);p.setPathEffect(oldEffect);
        }
    }
    ...
    public static final class Result{
        ...
        Result(Bitmap b,int e,int skipped,float[]points,List<Entity>document,Matrix view,Set<String>all,Set<String>visible,Set<String>layouts,String activeLayout,RectF contentBounds,float worldToContentScale,double millimetersPerUnit,String drawingUnitName,double globalLineTypeScale,Map<String,DxfLineStyle.Pattern>lineTypes,float[] modelExtents){bitmap=b;this.modelExtents=modelExtents==null?null:modelExtents.clone();entityCount=e;skippedCount=skipped;snapPoints=points;this.document=document;this.view=new Matrix(view);this.contentBounds=new RectF(contentBounds);this.worldToContentScale=worldToContentScale;this.millimetersPerUnit=millimetersPerUnit;this.drawingUnitName=drawingUnitName;this.globalLineTypeScale=safeLineTypeScale(globalLineTypeScale);this.lineTypes=Collections.unmodifiableMap(new LinkedHashMap<>(lineTypes));layerNames=Collections.unmodifiableSet(new TreeSet<>(all));visibleLayers=Collections.unmodifiableSet(new TreeSet<>(visible));layoutNames=Collections.unmodifiableSet(new LinkedHashSet<>(layouts));this.activeLayout=activeLayout;layerCount=all.size();ArrayList<LayerEntity>visibleList=new ArrayList<>();for(Entity wrapped:document){LayerEntity layer=(LayerEntity)wrapped;if(activeLayout.equals(layer.layout)&&visibleLayers.contains(layer.layer))visibleList.add(layer);}visibleDocument=Collections.unmodifiableList(visibleList);spatialIndex=new SpatialIndex(visibleList);ArrayList<LayerEntity>sourceList=new ArrayList<>();android.util.SparseArray<LayerEntity>sourceMap=new android.util.SparseArray<>();for(Entity entity:document){LayerEntity layer=(LayerEntity)entity;if(layer.sourceRange==null||layer.sourceEditWorld==null)continue;sourceList.add(layer);sourceMap.put(layer.sourceId,layer);}editableSourceLayers=Collections.unmodifiableList(sourceList);sourceLayerById=sourceMap;LinkedHashSet<String>fallbacks=new LinkedHashSet<>();for(Entity wrapped:document){Entity raw=((LayerEntity)wrapped).entity;if(raw instanceof Transformed)raw=((Transformed)raw).entity;if(raw instanceof Label){DxfTextStyle.Style st=((Label)raw).style;if(st.usesShx()&&!st.fontFile.isEmpty())fallbacks.add(st.fontFile);}else if(raw instanceof MTextLabel){MTextLabel mt=(MTextLabel)raw;if(mt.style.usesShx()&&!mt.style.fontFile.isEmpty())fallbacks.add(mt.style.fontFile);for(DxfMText.Run run:mt.rich.runs){String f=run.font==null?"":run.font.trim();if(run.usesShxFont())fallbacks.add(f);}}}fontFallbacks=Collections.unmodifiableSet(fallbacks);boolean shape=false;for(DxfLineStyle.Pattern p:lineTypes.values())if(p!=null&&p.requiresExternalShape()){shape=true;break;}externalShapeFallback=shape;}
        public Result withVisibleLayers(Set<String>selected)throws IOException{Set<String>visible=new HashSet<>(selected);visible.retainAll(layerNames);Result r=renderLayout(document,layerNames,visible,layoutNames,activeLayout,skippedCount,millimetersPerUnit,drawingUnitName,globalLineTypeScale,lineTypes,bitmap!=null,modelExtents);r.conversionWarnings=conversionWarnings;return r;}
        public Result withLayout(String selected)throws IOException{String layout=findLayout(layoutNames,selected);Result r=renderLayout(document,layerNames,visibleLayers,layoutNames,layout,skippedCount,millimetersPerUnit,drawingUnitName,globalLineTypeScale,lineTypes,bitmap!=null,modelExtents);r.conversionWarnings=conversionWarnings;return r;}
        private SourceEntity sourceEntity(LayerEntity layer){if(layer==null||layer.sourceRange==null||layer.sourceEditWorld==null)return null;CadEdit content=mapEditToContent(layer.sourceEditWorld,view);return new SourceEntity(layer.sourceId,layer.sourceRange,layer.sourceType,layer.layer,layer.layout,layer.color,layer.lineType,layer.lineTypeScale,layer.lineWeight,content);}
        public int editableSourceCount(){int count=0;for(LayerEntity layer:editableSourceLayers)if(activeLayout.equals(layer.layout)&&visibleLayers.contains(layer.layer))count++;return count;}
        public SourceEntity sourceById(int id){return sourceEntity(sourceLayerById.get(id));}
        public List<SourceEntity>editableSources(){return Collections.unmodifiableList(new AbstractList<SourceEntity>(){public SourceEntity get(int index){return sourceEntity(editableSourceLayers.get(index));}public int size(){return editableSourceLayers.size();}});}
        public boolean isSourceVisible(int id){LayerEntity layer=sourceLayerById.get(id);return layer!=null&&activeLayout.equals(layer.layout)&&visibleLayers.contains(layer.layer);}
        public SourceEntity findEditableSource(float x,float y,float tolerance,Set<Integer>hiddenIds){
            PointF world=drawingPointFromContent(x,y);float best=(float)drawingDistanceFromContent(Math.max(0f,tolerance));LayerEntity found=null;Set<Integer>hidden=hiddenIds==null?Collections.emptySet():hiddenIds;
            for(LayerEntity layer:editableSourceLayers){if(!activeLayout.equals(layer.layout)||hidden.contains(layer.sourceId)||!visibleLayers.contains(layer.layer))continue;float distance=layer.sourceEditWorld.hitDistance(world.x,world.y),allowed="TEXT".equals(layer.sourceType)?best*1.75f:best;if(distance<=allowed&&(found==null||distance<best)){best=distance;found=layer;}}
    ...
    private static final class SpatialIndex{
        private static final int MIN_INDEXED_ENTITIES=600,MAX_CELLS_PER_ENTITY=96;
        private static long nextQueryMark=1L;
        private final List<LayerEntity>all;private final RectF bounds;private final ArrayList<LayerEntity>[]cells;private final ArrayList<LayerEntity>overflow=new ArrayList<>();private final int grid;private final float cellW,cellH;
        @SuppressWarnings("unchecked") SpatialIndex(List<LayerEntity>source){
    ...
    private static boolean isUnboundedEntity(Entity entity){if(entity instanceof XLine)return true;if(entity instanceof Transformed)return isUnboundedEntity(((Transformed)entity).entity);return false;}
    private static boolean drawWipeoutMask(Canvas canvas,Paint paint,Matrix outer,Entity wrapped,boolean print){
        Entity raw=wrapped;Matrix actual=new Matrix(outer);
        while(raw instanceof Transformed){Transformed t=(Transformed)raw;Matrix combined=new Matrix();combined.setConcat(actual,t.matrix);actual=combined;raw=t.entity;}
        if(!(raw instanceof WipeoutEntity))return false;((WipeoutEntity)raw).drawMask(canvas,paint,actual,print);return true;
    }
    ...
    static Result blankDrawing(){
        ...
        LinkedHashSet<String> layers=new LinkedHashSet<>();layers.add("0");return new Result(null,0,0,new float[0],document,view,layers,layers,layouts,DxfBlocks.MODEL_LAYOUT,bounds,1f,1d,"birim",1d,Collections.emptyMap(),null);
    }
    ...
    private static Result renderStreaming(File file,boolean makePreview)throws IOException{
        ...
        return renderLayout(entities,layers,layers,layouts,initial,expanded.skipped,mm,unitName,expanded.globalLineTypeScale,expanded.lineTypes,makePreview,expanded.modelExtents);
    }

    private static Result renderLayout(List<Entity>document,Set<String>all,Set<String>visible,Set<String>layouts,String requested,int skipped,double mm,String unitName,double global,Map<String,DxfLineStyle.Pattern>lineTypes,boolean makePreview,float[] modelExtents)throws IOException{
        ...
        // DWG model extents exclude detached schematics far outside the main sheet.
        // Accept them for initial fit only when nearly all model entities belong inside.
        if(DxfBlocks.MODEL_LAYOUT.equalsIgnoreCase(active)&&modelExtents!=null&&modelExtents.length==4){
            RectF preferred=new RectF(modelExtents[0],modelExtents[1],modelExtents[2],modelExtents[3]);
            if(Float.isFinite(preferred.left)&&Float.isFinite(preferred.top)&&Float.isFinite(preferred.right)&&Float.isFinite(preferred.bottom)&&preferred.width()>0f&&preferred.height()>0f){
                int inside=0,total=0;RectF one=new RectF();
                for(int i=0,step=Math.max(1,layoutEntities.size()/4096);i<layoutEntities.size();i+=step){one.set(Float.MAX_VALUE,Float.MAX_VALUE,-Float.MAX_VALUE,-Float.MAX_VALUE);layoutEntities.get(i).bounds(one);if(!Float.isFinite(one.left)||!Float.isFinite(one.right)||one.right<one.left||one.bottom<one.top)continue;total++;if(preferred.contains(one.centerX(),one.centerY()))inside++;}
                if(total>0&&inside>=total*.9f)b.set(preferred);
            }
        }
        float s=Math.min((SIZE-2f*MARGIN)/b.width(),(SIZE-2f*MARGIN)/b.height());Matrix view=new Matrix();view.postTranslate(-b.left,-b.bottom);view.postScale(s,-s);view.postTranslate(MARGIN+(SIZE-2*MARGIN-b.width()*s)/2f,MARGIN+(SIZE-2*MARGIN-b.height()*s)/2f);RectF contentBounds=new RectF(b);view.mapRect(contentBounds);ArrayList<Entity>shown=new ArrayList<>();for(Entity e:layoutEntities){LayerEntity layer=(LayerEntity)e;if(visible.contains(layer.layer))shown.add(e);}Bitmap bitmap=makePreview?Bitmap.createBitmap(PREVIEW_SIZE,PREVIEW_SIZE,Bitmap.Config.RGB_565):null;
        try{if(bitmap!=null){Canvas canvas=new Canvas(bitmap);canvas.drawColor(SCREEN_BG);Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG);paint.setStyle(Paint.Style.STROKE);float previewFactor=PREVIEW_SIZE/(float)SIZE;Matrix contentToPreview=new Matrix();contentToPreview.setScale(previewFactor,previewFactor);Matrix previewView=new Matrix();previewView.setConcat(contentToPreview,view);drawViewportModels(canvas,paint,previewView,document,active,visible,false,false,global);for(Entity e:shown){FileTransfer.checkCancelled();((LayerEntity)e).drawStyled(canvas,paint,previewView,false,false,global);}}FileTransfer.checkCancelled();int displayCount=0;for(Entity entity:shown){LayerEntity layer=(LayerEntity)entity;if(!"MUSACAD_BLANK".equals(layer.sourceType))displayCount++;}return new Result(bitmap,displayCount,skipped,snapPoints(shown,view),document,view,all,visible,layouts,active,contentBounds,s,mm,unitName,global,lineTypes,modelExtents);}catch(IOException|RuntimeException|OutOfMemoryError e){if(bitmap!=null)bitmap.recycle();throw e;}
    }
    ...
    private static void drawContentEdit(Canvas canvas,Paint paint,Matrix matrix,CadEdit edit){
        ...
        switch(edit.type){
            ...
            case ELLIPSE:if(v.length>=6){float a=(float)Math.hypot(v[2]-v[0],v[3]-v[1]),b=(float)Math.hypot(v[4]-v[0],v[5]-v[1]),angle=(float)Math.toDegrees(Math.atan2(v[3]-v[1],v[2]-v[0]));int save=canvas.save();canvas.rotate(angle,v[0],v[1]);canvas.drawOval(new RectF(v[0]-a,v[1]-b,v[0]+a,v[1]+b),paint);canvas.restoreToCount(save);}break;
            case POINT:if(v.length>=2){float r=5f;canvas.drawLine(v[0]-r,v[1],v[0]+r,v[1],paint);canvas.drawLine(v[0],v[1]-r,v[0],v[1]+r,paint);canvas.drawCircle(v[0],v[1],r*.55f,paint);}break;
            case XLINE:if(v.length>=4){float dx=v[2]-v[0],dy=v[3]-v[1],len=(float)Math.hypot(dx,dy);if(len>1e-6f){dx/=len;dy/=len;float reach=(float)Math.hypot(canvas.getWidth(),canvas.getHeight())*2f;canvas.drawLine(v[0]-dx*reach,v[1]-dy*reach,v[0]+dx*reach,v[1]+dy*reach,paint);}}break;
