...
public final class DxfParser {
    private static final int SIZE=2400,MARGIN=80,PREVIEW_SIZE=1200;
    ...
    private static Result renderLayout(List<Entity>document,Set<String>all,Set<String>visible,Set<String>layouts,String requested,int skipped,double mm,String unitName,double global,Map<String,DxfLineStyle.Pattern>lineTypes,boolean makePreview)throws IOException{
        ...
        try{if(bitmap!=null){Canvas canvas=new Canvas(bitmap);canvas.drawColor(Color.rgb(18,24,30));Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG);paint.setStyle(Paint.Style.STROKE);float previewFactor=PREVIEW_SIZE/(float)SIZE;Matrix contentToPreview=new Matrix();contentToPreview.setScale(previewFactor,previewFactor);Matrix previewView=new Matrix();previewView.setConcat(contentToPreview,view);drawViewportModels(canvas,paint,previewView,document,active,visible,false,false,global);for(Entity e:shown){FileTransfer.checkCancelled();((LayerEntity)e).drawStyled(canvas,paint,previewView,false,false,global);}}FileTransfer.checkCancelled();int displayCount=0;for(Entity entity:shown){LayerEntity layer=(LayerEntity)entity;if(!"MUSACAD_BLANK".equals(layer.sourceType))displayCount++;}return new Result(bitmap,displayCount,skipped,snapPoints(shown,view),document,view,all,visible,layouts,active,contentBounds,s,mm,unitName,global,lineTypes);}catch(IOException|RuntimeException|OutOfMemoryError e){if(bitmap!=null)bitmap.recycle();throw e;}
    }
