...
public final class DxfParser {
    ...
    public static final class SourceEntity{
        public final int sourceId;public final SourceRange range;public final String type,layer,lineType,rawLineType,layout;public final int color,colorMode,colorValue,lineWeight,rawLineWeight;public final double lineTypeScale;private final CadEdit prototype;
        SourceEntity(int sourceId,SourceRange range,String type,String layer,String layout,int color,int colorMode,int colorValue,String lineType,String rawLineType,double lineTypeScale,int lineWeight,int rawLineWeight,CadEdit prototype){
            this.sourceId=sourceId;this.range=range;this.type=type;this.layer=layer;this.layout=layout;this.color=color;this.colorMode=colorMode;this.colorValue=colorValue;
            this.lineType=DxfLineStyle.normalizeName(lineType);this.rawLineType=DxfLineStyle.normalizeName(rawLineType);this.lineTypeScale=lineTypeScale;this.lineWeight=lineWeight;this.rawLineWeight=rawLineWeight;this.prototype=prototype;
        }
    ...
    public static final class Result{
        public final Bitmap bitmap;public final float[] snapPoints;public final int entityCount,layerCount,skippedCount;public int conversionWarnings;public final Set<String>layerNames,visibleLayers,layoutNames,fontFallbacks;public final String activeLayout;public final boolean externalShapeFallback;
        private final List<Entity>document;private final Matrix view;private final RectF contentBounds;private final float worldToContentScale;private final double millimetersPerUnit;private final String drawingUnitName;private final double globalLineTypeScale;private final Map<String,DxfLineStyle.Pattern>lineTypes;private final Map<String,DxfTextStyle.Style>textStyles;private final Map<String,Integer>layerColors,layerLineWeights;private final Map<String,String>layerLineTypes;private final List<SourceEntity>editableSources;private final Map<Integer,SourceEntity>sourceById;
        Result(Bitmap b,int e,int skipped,float[]points,List<Entity>document,Matrix view,Set<String>all,Set<String>visible,Set<String>layouts,String activeLayout,RectF contentBounds,float worldToContentScale,double millimetersPerUnit,String drawingUnitName,double globalLineTypeScale,Map<String,DxfLineStyle.Pattern>lineTypes,Map<String,DxfTextStyle.Style>textStyles,Map<String,Integer>layerColors,Map<String,String>layerLineTypes,Map<String,Integer>layerLineWeights){bitmap=b;entityCount=e;skippedCount=skipped;snapPoints=points;this.document=document;this.view=new Matrix(view);this.contentBounds=new RectF(contentBounds);this.worldToContentScale=worldToContentScale;this.millimetersPerUnit=millimetersPerUnit;this.drawingUnitName=drawingUnitName;this.globalLineTypeScale=safeLineTypeScale(globalLineTypeScale);this.lineTypes=Collections.unmodifiableMap(new LinkedHashMap<>(lineTypes));this.textStyles=Collections.unmodifiableMap(new LinkedHashMap<>(textStyles));this.layerColors=Collections.unmodifiableMap(new LinkedHashMap<>(layerColors));this.layerLineTypes=Collections.unmodifiableMap(new LinkedHashMap<>(layerLineTypes));this.layerLineWeights=Collections.unmodifiableMap(new LinkedHashMap<>(layerLineWeights));layerNames=Collections.unmodifiableSet(new TreeSet<>(all));visibleLayers=Collections.unmodifiableSet(new TreeSet<>(visible));layoutNames=Collections.unmodifiableSet(new LinkedHashSet<>(layouts));this.activeLayout=activeLayout;layerCount=all.size();ArrayList<SourceEntity>sourceList=new ArrayList<>();LinkedHashMap<Integer,SourceEntity>sourceMap=new LinkedHashMap<>();for(Entity entity:document){LayerEntity layer=(LayerEntity)entity;if(layer.sourceRange==null||layer.sourceEditWorld==null)continue;CadEdit ce=mapEditToContent(layer.sourceEditWorld,this.view);SourceEntity source=new SourceEntity(layer.sourceId,layer.sourceRange,layer.sourceType,layer.layer,layer.layout,layer.color,layer.rawColorMode,layer.rawColorValue,layer.lineType,layer.rawLineType,layer.lineTypeScale,layer.lineWeight,layer.rawLineWeight,ce);sourceList.add(source);sourceMap.put(source.sourceId,source);}editableSources=Collections.unmodifiableList(sourceList);sourceById=Collections.unmodifiableMap(sourceMap);LinkedHashSet<String>fallbacks=new LinkedHashSet<>();for(Entity wrapped:document){Entity raw=((LayerEntity)wrapped).entity;if(raw instanceof Transformed)raw=((Transformed)raw).entity;if(raw instanceof Label){DxfTextStyle.Style st=((Label)raw).style;if(st.usesShx()&&!st.fontFile.isEmpty())fallbacks.add(st.fontFile);}else if(raw instanceof MTextLabel){MTextLabel mt=(MTextLabel)raw;if(mt.style.usesShx()&&!mt.style.fontFile.isEmpty())fallbacks.add(mt.style.fontFile);for(DxfMText.Run run:mt.rich.runs){String f=run.font==null?"":run.font.trim();if(run.usesShxFont())fallbacks.add(f);}}}fontFallbacks=Collections.unmodifiableSet(fallbacks);boolean shape=false;for(DxfLineStyle.Pattern p:lineTypes.values())if(p!=null&&p.requiresExternalShape()){shape=true;break;}externalShapeFallback=shape;}
        public Set<String> lineTypeNames(){TreeSet<String> names=new TreeSet<>(lineTypes.keySet());names.add(DxfLineStyle.BYLAYER);names.add(DxfLineStyle.BYBLOCK);return Collections.unmodifiableSet(names);}
        public Set<String> textStyleNames(){return Collections.unmodifiableSet(new LinkedHashSet<>(textStyles.keySet()));}
        public DxfTextStyle.Style textStyle(String name){return DxfTextStyle.resolve(textStyles,name);}

        public int layerColor(String name){
            String wanted=name==null?"0":name;for(Map.Entry<String,Integer> e:layerColors.entrySet())if(e.getKey().equalsIgnoreCase(wanted))return e.getValue();return Color.WHITE;
        }
        public int layerLineWeight(String name){
            String wanted=name==null?"0":name;for(Map.Entry<String,Integer> e:layerLineWeights.entrySet())if(e.getKey().equalsIgnoreCase(wanted))return e.getValue();return DxfLineStyle.DEFAULT_LINEWEIGHT;
        }
        public String layerLineType(String name){
            String wanted=name==null?"0":name;for(Map.Entry<String,String> e:layerLineTypes.entrySet())if(e.getKey().equalsIgnoreCase(wanted))return e.getValue();return DxfLineStyle.CONTINUOUS;
        }
        public int resolveColor(String layerName,int colorMode,int colorValue){
            if(colorMode==CadEdit.COLOR_ACI)return DxfColor.aciArgb(colorValue);
            if(colorMode==CadEdit.COLOR_TRUECOLOR)return 0xFF000000|(colorValue&0x00FFFFFF);
            if(colorMode==CadEdit.COLOR_BYLAYER)return layerColor(layerName);
            return Color.WHITE;
        }
        public int resolveLineWeight(String layerName,int raw){
            if(raw==DxfLineStyle.LW_BYLAYER)return layerLineWeight(layerName);
            if(raw==DxfLineStyle.LW_BYBLOCK||raw==DxfLineStyle.LW_DEFAULT)return DxfLineStyle.DEFAULT_LINEWEIGHT;
            return DxfLineStyle.normalizeWeight(raw,DxfLineStyle.DEFAULT_LINEWEIGHT);
        }
        public String resolveLineType(String layerName,String raw){
            String type=DxfLineStyle.normalizeName(raw);if(DxfLineStyle.BYLAYER.equals(type))return layerLineType(layerName);if(DxfLineStyle.BYBLOCK.equals(type))return DxfLineStyle.CONTINUOUS;return type;
        }
        public Result withVisibleLayers(Set<String>selected)throws IOException{Set<String>visible=new HashSet<>(selected);visible.retainAll(layerNames);Result r=renderLayout(document,layerNames,visible,layoutNames,activeLayout,skippedCount,millimetersPerUnit,drawingUnitName,globalLineTypeScale,lineTypes,textStyles,layerColors,layerLineTypes,layerLineWeights);r.conversionWarnings=conversionWarnings;return r;}
        public Result withLayout(String selected)throws IOException{String layout=findLayout(layoutNames,selected);Result r=renderLayout(document,layerNames,visibleLayers,layoutNames,layout,skippedCount,millimetersPerUnit,drawingUnitName,globalLineTypeScale,lineTypes,textStyles,layerColors,layerLineTypes,layerLineWeights);r.conversionWarnings=conversionWarnings;return r;}
        ...
        public SourceEntity findEditableSource(float x,float y,float tolerance,Set<Integer>hiddenIds){float best=Math.max(0f,tolerance);SourceEntity found=null;Set<Integer>hidden=hiddenIds==null?Collections.emptySet():hiddenIds;for(SourceEntity s:editableSources){if(!activeLayout.equals(s.layout)||hidden.contains(s.sourceId)||!visibleLayers.contains(s.layer))continue;float allowed="TEXT".equals(s.type)?best*1.75f:best;CadEdit p=s.prototype;if(x<p.minX()-allowed||x>p.maxX()+allowed||y<p.minY()-allowed||y>p.maxY()+allowed)continue;float distance=s.hitDistance(x,y);if(distance<=allowed&&(found==null||distance<best)){best=distance;found=s;}}return found;}
        public void drawVector(Canvas canvas,Matrix imageMatrix){drawVector(canvas,imageMatrix,Collections.emptySet(),false);}
        public void drawVector(Canvas canvas,Matrix imageMatrix,Set<Integer>hiddenIds){drawVector(canvas,imageMatrix,hiddenIds,false);}
        public void drawVector(Canvas canvas,Matrix imageMatrix,Set<Integer>hiddenIds,boolean fast){
            ...
            RectF visibleWorld=null;Rect clip=canvas.getClipBounds();Matrix inv=new Matrix();if(clip!=null&&combined.invert(inv)){visibleWorld=new RectF(clip);inv.mapRect(visibleWorld);float pad=Math.max(1f,Math.max(visibleWorld.width(),visibleWorld.height())*.03f);visibleWorld.inset(-pad,-pad);}
            Set<Integer>hidden=hiddenIds==null?Collections.emptySet():hiddenIds;
            for(Entity entity:document){LayerEntity layer=(LayerEntity)entity;if(activeLayout.equals(layer.layout)&&visibleLayers.contains(layer.layer)&&!hidden.contains(layer.sourceId)&&layer.visibleIn(visibleWorld))layer.drawStyled(canvas,paint,combined,false,false,globalLineTypeScale,fast);}
        }
        ...
        public void drawSourceReplacement(Canvas canvas,Matrix contentToTarget,SourceReplacement r,boolean print,boolean mono){
            if(canvas==null||contentToTarget==null||r==null||!isSourceVisible(r.sourceId))return;Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG);paint.setStyle(Paint.Style.STROKE);
            int color=resolveColor(r.layer,r.colorMode,r.colorValue),weight=resolveLineWeight(r.layer,r.lineWeight);String type=resolveLineType(r.layer,r.lineType);
            paint.setColor(mono?Color.BLACK:(print?paperColor(color):color));paint.setStrokeWidth(print?DxfLineStyle.printStrokePoints(weight):DxfLineStyle.screenStroke(weight));
            DxfLineStyle.Pattern pattern=lineTypes.get(DxfLineStyle.normalizeName(type));if(pattern==null)pattern=lineTypes.get(DxfLineStyle.CONTINUOUS);double pixels=matrixScale(contentToTarget)*worldToContentScale;DxfLineStyle.Dash dash=pattern==null?null:pattern.dash(pixels,globalLineTypeScale,r.lineTypeScale,1d);
            paint.setPathEffect(dash==null?null:new DashPathEffect(dash.intervals,dash.phase));drawContentEdit(canvas,paint,contentToTarget,r.edit);paint.setPathEffect(null);
        }
    ...
    private static final class LayerEntity implements Entity{
        final Entity entity;final String layer,layout;final int color;final String lineType,rawLineType;final DxfLineStyle.Pattern linePattern;final double lineTypeScale,blockScale;final int lineWeight,rawLineWeight,rawColorMode,rawColorValue,sourceId;final SourceRange sourceRange;final String sourceType;final CadEdit sourceEditWorld;final RectF worldBounds;
        LayerEntity(Entity e,String layer,String layout,int color,String lineType,DxfLineStyle.Pattern pattern,double scale,int weight,int rawColorMode,int rawColorValue,String rawLineType,int rawLineWeight,double blockScale,int sourceId,SourceRange range,String type,CadEdit edit){entity=e;this.layer=layer;this.layout=layout;this.color=color;this.lineType=DxfLineStyle.normalizeName(lineType);this.rawColorMode=rawColorMode;this.rawColorValue=rawColorValue;this.rawLineType=DxfLineStyle.normalizeName(rawLineType);this.rawLineWeight=rawLineWeight;linePattern=pattern;lineTypeScale=safeLineTypeScale(scale);lineWeight=weight;this.blockScale=blockScale;this.sourceId=sourceId;sourceRange=range;sourceType=type;sourceEditWorld=edit;RectF b=new RectF(Float.MAX_VALUE,Float.MAX_VALUE,-Float.MAX_VALUE,-Float.MAX_VALUE);e.bounds(b);worldBounds=Float.isFinite(b.left)&&Float.isFinite(b.top)&&b.right>=b.left&&b.bottom>=b.top?new RectF(b):null;}
        boolean visibleIn(RectF visible){return visible==null||worldBounds==null||RectF.intersects(worldBounds,visible);}
        public void bounds(RectF b){if(worldBounds!=null){add(b,worldBounds.left,worldBounds.top);add(b,worldBounds.right,worldBounds.bottom);}else entity.bounds(b);}
        public void draw(Canvas c,Paint p,Matrix m){drawStyled(c,p,m,false,false,1d,false);}
        void drawStyled(Canvas c,Paint p,Matrix m,boolean print,boolean mono,double global){drawStyled(c,p,m,print,mono,global,false);}
        void drawStyled(Canvas c,Paint p,Matrix m,boolean print,boolean mono,double global,boolean fast){p.setColor(mono?Color.BLACK:(print?paperColor(color):color));p.setStrokeWidth(print?DxfLineStyle.printStrokePoints(lineWeight):DxfLineStyle.screenStroke(lineWeight));DxfLineStyle.Dash dash=fast?null:(linePattern==null?null:linePattern.dash(matrixScale(m),global,lineTypeScale,blockScale));p.setPathEffect(dash==null?null:new DashPathEffect(dash.intervals,dash.phase));entity.draw(c,p,m);p.setPathEffect(null);if(!fast)drawComplexLineText(c,p,m,entity,linePattern,global,lineTypeScale,blockScale);}
    }

    private static void drawComplexLineText(Canvas canvas,Paint paint,Matrix outer,Entity wrapped,DxfLineStyle.Pattern pattern,double global,double entityScale,double blockScale){
        if(pattern==null||!pattern.hasRenderableComplexText())return;Entity raw=wrapped;Matrix actual=new Matrix(outer);if(raw instanceof Transformed){Transformed transformed=(Transformed)raw;Matrix combined=new Matrix();combined.setConcat(outer,transformed.matrix);actual=combined;raw=transformed.entity;}double factor=matrixScale(outer)*safeLineTypeScale(global)*safeLineTypeScale(entityScale)*safeLineTypeScale(blockScale);if(!Double.isFinite(factor)||factor<=0d)return;
        if(raw instanceof Line){Line line=(Line)raw;float[]v={line.x1,line.y1,line.x2,line.y2};actual.mapPoints(v);DxfComplexLineAndroid.drawSegment(canvas,paint,pattern,v[0],v[1],v[2],v[3],factor);return;}
        if(raw instanceof Poly){Poly poly=(Poly)raw;if(poly.pts.size()<2)return;float[]v=new float[poly.pts.size()*2];for(int i=0;i<poly.pts.size();i++){v[i*2]=poly.pts.get(i).x;v[i*2+1]=poly.pts.get(i).y;}actual.mapPoints(v);double offset=0d;for(int i=0;i+3<v.length;i+=2){DxfComplexLineAndroid.drawSegment(canvas,paint,pattern,v[i],v[i+1],v[i+2],v[i+3],factor,offset);offset+=Math.hypot(v[i+2]-v[i],v[i+3]-v[i+1]);}if(poly.closed&&v.length>=4){float x1=v[v.length-2],y1=v[v.length-1],x2=v[0],y2=v[1];if(Math.hypot(x2-x1,y2-y1)>1e-4)DxfComplexLineAndroid.drawSegment(canvas,paint,pattern,x1,y1,x2,y2,factor,offset);}
        }
    }
    ...
    private static Result renderStreaming(File file)throws IOException{
        ...
        final class Collector implements DxfBlocks.Sink{
            ...
            private void add(DxfBlocks.Placement item,Entity entity)throws IOException{
                ...
                int rawMode=CadEdit.COLOR_BYLAYER,rawValue=7,rawWeight=DxfLineStyle.LW_BYLAYER;String rawType=DxfLineStyle.BYLAYER;
                if(item.directRoot){
                    rawMode=rawColorMode(item.record);rawValue=rawColorValue(item.record,rawMode);rawType=item.record.text(6,DxfLineStyle.BYLAYER);rawWeight=item.record.integer(370,DxfLineStyle.LW_BYLAYER);
                }
                entities.add(new LayerEntity(placed,item.layer,item.layout,item.color,item.lineType,pattern,item.lineTypeScale,item.lineWeight,rawMode,rawValue,rawType,rawWeight,item.blockScale,sourceId,range,item.record.type,sourceEdit));layers.add(item.layer);layouts.add(item.layout);
            }
        ...
        return renderLayout(entities,layers,layers,layouts,initial,expanded.skipped,mm,unitName,expanded.globalLineTypeScale,expanded.lineTypes,expanded.textStyles,expanded.layerColors,expanded.layerLineTypes,expanded.layerLineWeights);
    }

    private static int rawColorMode(DxfBlocks.Record record)throws IOException{
        if(record.has(420))return CadEdit.COLOR_TRUECOLOR;int aci=record.integer(62,DxfColor.BYLAYER);
        if(aci==DxfColor.BYBLOCK)return CadEdit.COLOR_BYBLOCK;if(aci==DxfColor.BYLAYER)return CadEdit.COLOR_BYLAYER;return CadEdit.COLOR_ACI;
    }
    private static int rawColorValue(DxfBlocks.Record record,int mode)throws IOException{
        if(mode==CadEdit.COLOR_TRUECOLOR){try{long raw=Long.parseLong(record.text(420,"0").trim());return (int)raw&0x00FFFFFF;}catch(NumberFormatException e){throw new IOException("Geçersiz DXF TrueColor",e);}}
        if(mode==CadEdit.COLOR_ACI)return Math.max(1,Math.min(255,Math.abs(record.integer(62,DxfColor.DEFAULT_ACI))));
        return 7;
    }

    private static Result renderLayout(List<Entity>document,Set<String>all,Set<String>visible,Set<String>layouts,String requested,int skipped,double mm,String unitName,double global,Map<String,DxfLineStyle.Pattern>lineTypes,Map<String,DxfTextStyle.Style>textStyles,Map<String,Integer>layerColors,Map<String,String>layerLineTypes,Map<String,Integer>layerLineWeights)throws IOException{
