...
public final class DxfParser {
    ...
    public static final class Result{
        ...
        public int editableSourceCount(){int count=0;for(LayerEntity layer:editableSourceLayers)if(activeLayout.equals(layer.layout)&&layer.isVisible(visibleLayerKeys))count++;return count;}
        ...
        public boolean isSourceVisible(int id){LayerEntity layer=sourceLayerById.get(id);return layer!=null&&activeLayout.equals(layer.layout)&&layer.isVisible(visibleLayerKeys);}
        public SourceEntity findEditableSource(float x,float y,float tolerance,Set<Integer>hiddenIds){
            ...
            for(LayerEntity layer:editableSourceLayers){if(!activeLayout.equals(layer.layout)||hidden.contains(layer.sourceId)||!layer.isVisible(visibleLayerKeys))continue;float distance=layer.sourceEditWorld.hitDistance(world.x,world.y),allowed="TEXT".equals(layer.sourceType)?best*1.75f:best;if(distance<=allowed&&(found==null||distance<best)){best=distance;found=layer;}}
        ...
        public void drawVectorForPrint(Canvas canvas,Matrix contentToPage,boolean monochrome,Set<Integer>hiddenIds){Paint paint=new Paint(Paint.ANTI_ALIAS_FLAG);paint.setStyle(Paint.Style.STROKE);Matrix combined=new Matrix();combined.setConcat(contentToPage,view);drawViewportModels(canvas,paint,combined,document,activeLayout,visibleLayerKeys,true,monochrome,globalLineTypeScale);Set<Integer>hidden=hiddenIds==null?Collections.emptySet():hiddenIds;for(Entity entity:document){LayerEntity layer=(LayerEntity)entity;if(!activeLayout.equals(layer.layout)||!layer.isVisible(visibleLayerKeys)||hidden.contains(layer.sourceId))continue;layer.drawStyled(canvas,paint,combined,true,monochrome,globalLineTypeScale);}}
        ...
        public List<DxfViewport.View> viewports(){
            ...
            for(Entity wrapped:document){
                ...
                if(!activeLayout.equals(layer.layout)||!layer.isVisible(visibleLayerKeys))continue;
    ...
    static Result blankDrawing(){
        ...
        document.add(new LayerEntity(canvasEntity,"0",DxfBlocks.MODEL_LAYOUT,Color.WHITE,DxfLineStyle.CONTINUOUS,null,1d,-1,1d,-1,null,"MUSACAD_BLANK",null,new String[]{"0"}));
    ...
    private static Result renderStreaming(File file,boolean makePreview)throws IOException{
        ...
        final class Collector implements DxfBlocks.Sink{
            ...
            private void add(DxfBlocks.Placement item,Entity entity)throws IOException{
                ...
                entities.add(new LayerEntity(placed,item.layer,item.layout,item.color,item.lineType,pattern,item.lineTypeScale,item.lineWeight,item.blockScale,sourceId,range,item.record.type,sourceEdit,item.visibilityLayerKeys));layers.add(item.layer);layouts.add(item.layout);
            }
        ...
        if(entities.isEmpty())return null;layers.addAll(expanded.layerColors.keySet());if(layouts.isEmpty())layouts.addAll(expanded.layoutNames);if(layouts.isEmpty())layouts.add(DxfBlocks.MODEL_LAYOUT);
        LinkedHashSet<String>initialVisible=new LinkedHashSet<>();for(String layer:layers)if(expanded.layerInitiallyVisible(layer))initialVisible.add(layer);
        ...
        return renderLayout(entities,layers,initialVisible,layouts,initial,expanded.skipped,mm,unitName,expanded.globalLineTypeScale,expanded.lineTypes,makePreview,expanded.modelExtents);
    }
