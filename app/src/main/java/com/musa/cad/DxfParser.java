...
public final class DxfParser {
    ...
    public static final class Result{
        ...
        public void drawVector(Canvas canvas,Matrix imageMatrix,Set<Integer>hiddenIds){
            ...
            for(LayerEntity layer:visibleDocument){if(hidden.contains(layer.sourceId)||!layer.intersects(visibleWorld))continue;layer.drawStyled(canvas,paint,combined,false,false,globalLineTypeScale);}
        }
    ...
    private static final class LayerEntity implements Entity{
        ...
        private RectF boundsCached(){if(cachedBounds!=null)return cachedBounds;RectF r=new RectF(Float.MAX_VALUE,Float.MAX_VALUE,-Float.MAX_VALUE,-Float.MAX_VALUE);entity.bounds(r);if(!Float.isFinite(r.left)||!Float.isFinite(r.top)||!Float.isFinite(r.right)||!Float.isFinite(r.bottom)||r.right<r.left||r.bottom<r.top){cachedBounds=new RectF();return cachedBounds;}if(r.right-r.left<.001f){r.left-=.5f;r.right+=.5f;}if(r.bottom-r.top<.001f){r.top-=.5f;r.bottom+=.5f;}cachedBounds=new RectF(r);return cachedBounds;}
        boolean intersects(RectF visible){if(unbounded||visible==null)return true;RectF r=boundsCached();return !r.isEmpty()&&RectF.intersects(r,visible);}
        public void bounds(RectF b){RectF r=boundsCached();if(r.isEmpty())return;add(b,r.left,r.top);add(b,r.right,r.bottom);}public void draw(Canvas c,Paint p,Matrix m){drawStyled(c,p,m,false,false,1d);}void drawStyled(Canvas c,Paint p,Matrix m,boolean print,boolean mono,double global){p.setColor(mono?Color.BLACK:(print?paperColor(color):color));p.setStrokeWidth(print?DxfLineStyle.printStrokePoints(lineWeight):DxfLineStyle.screenStroke(lineWeight));DxfLineStyle.Dash dash=linePattern==null?null:linePattern.dash(matrixScale(m),global,lineTypeScale,blockScale);p.setPathEffect(dash==null?null:new DashPathEffect(dash.intervals,dash.phase));entity.draw(c,p,m);p.setPathEffect(null);drawComplexLineText(c,p,m,entity,linePattern,global,lineTypeScale,blockScale);}
    }
