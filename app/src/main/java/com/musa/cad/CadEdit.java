...
public final class CadEdit {
    ...
    public final int colorMode,colorValue,lineWeight;
    public final String lineTypeName;

    private CadEdit(Type type,float[] xy,String text,float strokeWidth,boolean closed,float rotationDegrees){this(type,xy,text,strokeWidth,closed,rotationDegrees,"STANDARD","sans",false,0f,1f,0f,0,"0",COLOR_BYLAYER,7,DxfLineStyle.BYLAYER,DxfLineStyle.LW_BYLAYER);}
    private CadEdit(Type type,float[] xy,String text,float strokeWidth,boolean closed,float rotationDegrees,String textStyleName,String textFamilyHint,boolean textShx,float textHeight,float textWidthFactor,float textOblique,int textGenerationFlags){this(type,xy,text,strokeWidth,closed,rotationDegrees,textStyleName,textFamilyHint,textShx,textHeight,textWidthFactor,textOblique,textGenerationFlags,"0",COLOR_BYLAYER,7,DxfLineStyle.BYLAYER,DxfLineStyle.LW_BYLAYER);}
    private CadEdit(Type type,float[] xy,String text,float strokeWidth,boolean closed,float rotationDegrees,String textStyleName,String textFamilyHint,boolean textShx,float textHeight,float textWidthFactor,float textOblique,int textGenerationFlags,String layerName,int colorMode,int colorValue,String lineTypeName,int lineWeight){
        ...
        this.lineTypeName=DxfLineStyle.normalizeName(lineTypeName);this.lineWeight=rawLineWeight(lineWeight);
    }
    ...
    public CadEdit copy(){return new CadEdit(type,Arrays.copyOf(xy,xy.length),text,strokeWidth,closed,rotationDegrees,textStyleName,textFamilyHint,textShx,textHeight,textWidthFactor,textOblique,textGenerationFlags,layerName,colorMode,colorValue,lineTypeName,lineWeight);}
    public CadEdit translated(float dx,float dy){float[] out=xy.clone();for(int i=0;i+1<out.length;i+=2){out[i]+=dx;out[i+1]+=dy;}return new CadEdit(type,out,text,strokeWidth,closed,rotationDegrees,textStyleName,textFamilyHint,textShx,textHeight,textWidthFactor,textOblique,textGenerationFlags,layerName,colorMode,colorValue,lineTypeName,lineWeight);}
    ...
    public CadEdit withTextHeight(float height){return new CadEdit(type,xy.clone(),text,strokeWidth,closed,rotationDegrees,textStyleName,textFamilyHint,textShx,height,textWidthFactor,textOblique,textGenerationFlags,layerName,colorMode,colorValue,lineTypeName,lineWeight);}
    public CadEdit withTextStyle(String styleName,String familyHint,boolean shx,float height,float widthFactor){return new CadEdit(type,xy.clone(),text,strokeWidth,closed,rotationDegrees,styleName,familyHint,shx,height,widthFactor,textOblique,textGenerationFlags,layerName,colorMode,colorValue,lineTypeName,lineWeight);}
    public CadEdit withCadProperties(String layer,int mode,int value){return new CadEdit(type,xy.clone(),text,strokeWidth,closed,rotationDegrees,textStyleName,textFamilyHint,textShx,textHeight,textWidthFactor,textOblique,textGenerationFlags,layer,mode,value,lineTypeName,lineWeight);}
    public CadEdit withCadProperties(String layer,int mode,int value,String lineType,int weight){return new CadEdit(type,xy.clone(),text,strokeWidth,closed,rotationDegrees,textStyleName,textFamilyHint,textShx,textHeight,textWidthFactor,textOblique,textGenerationFlags,layer,mode,value,lineType,weight);}
