...
public final class CadEdit {
    ...
    /** Screen-oriented overlay width multiplier. DXF export keeps geometry as CAD entities. */
    public final float strokeWidth;
    /** Relevant for POLYLINE source geometry and export. */
    public final boolean closed;
    /** Content-space text rotation. Positive values follow Android canvas rotation. */
    ...
    private CadEdit(Type type,float[] xy,String text,float strokeWidth,boolean closed,float rotationDegrees){
        this(type,xy,text,strokeWidth,closed,rotationDegrees,DxfTextStyle.STANDARD,"sans",false,0f,1f,0f,0);
    }
    private CadEdit(Type type,float[] xy,String text,float strokeWidth,boolean closed,float rotationDegrees,String textStyleName,String textFamilyHint,boolean textShx,float textHeight,float textWidthFactor,float textOblique,int textGenerationFlags){
        ...
        this.textStyleName=textStyleName==null||textStyleName.trim().isEmpty()?DxfTextStyle.STANDARD:textStyleName.trim();
        this.textFamilyHint=textFamilyHint==null||textFamilyHint.trim().isEmpty()?"sans":textFamilyHint.trim();this.textShx=textShx;
        this.textHeight=Float.isFinite(textHeight)&&textHeight>0f?textHeight:0f;this.textWidthFactor=Float.isFinite(textWidthFactor)&&textWidthFactor>0f?textWidthFactor:1f;
        this.textOblique=Float.isFinite(textOblique)?textOblique:0f;this.textGenerationFlags=textGenerationFlags;
    }
    ...
    public static CadEdit styledText(float x,float y,String text,float rotationDegrees,String styleName,String familyHint,boolean shx,float height,float widthFactor,float oblique,int generationFlags){
    ...
    public CadEdit translated(float dx,float dy){
        float[] out=xy.clone();for(int i=0;i+1<out.length;i+=2){out[i]+=dx;out[i+1]+=dy;}
    ...
    public CadEdit rotated(float degrees,float pivotX,float pivotY){
        double r=Math.toRadians(degrees),co=Math.cos(r),si=Math.sin(r);float[] out=xy.clone();
        for(int i=0;i+1<out.length;i+=2){double x=out[i]-pivotX,y=out[i+1]-pivotY;out[i]=(float)(pivotX+x*co-y*si);out[i+1]=(float)(pivotY+x*si+y*co);}
        float textRotation=type==Type.TEXT?rotationDegrees+degrees:rotationDegrees;
    ...
    public CadEdit withTextHeight(float height){
        return new CadEdit(type,xy.clone(),text,strokeWidth,closed,rotationDegrees,textStyleName,textFamilyHint,textShx,height,textWidthFactor,textOblique,textGenerationFlags);
    }
    ...
    /** Approximate content-space hit distance used by source-entity selection. */
    public float hitDistance(float x,float y){
        switch(type){
            ...
            case TEXT:return (float)Math.hypot(x-xy[0],y-xy[1]);
