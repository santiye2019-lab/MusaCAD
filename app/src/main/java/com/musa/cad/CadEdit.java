...
public final class CadEdit {
    ...
    private CadEdit(Type type,float[] xy,String text,float strokeWidth,boolean closed,float rotationDegrees){
        this.type=type;this.xy=xy;this.text=text;this.strokeWidth=Math.max(1f,strokeWidth);this.closed=closed;
    ...
    public CadEdit copy(){return new CadEdit(type,Arrays.copyOf(xy,xy.length),text,strokeWidth,closed,rotationDegrees);}

    public CadEdit translated(float dx,float dy){
        float[] out=xy.clone();for(int i=0;i+1<out.length;i+=2){out[i]+=dx;out[i+1]+=dy;}
        return new CadEdit(type,out,text,strokeWidth,closed,rotationDegrees);
    }
    ...
    public CadEdit rotated(float degrees,float pivotX,float pivotY){
        ...
        for(int i=0;i+1<out.length;i+=2){
            double x=out[i]-pivotX,y=out[i+1]-pivotY;
            out[i]=(float)(pivotX+x*co-y*si);out[i+1]=(float)(pivotY+x*si+y*co);
        }
    ...
    public float hitDistance(float x,float y){
        switch(type){
            ...
            case RECTANGLE:{
                float l=Math.min(xy[0],xy[2]),r=Math.max(xy[0],xy[2]),t=Math.min(xy[1],xy[3]),b=Math.max(xy[1],xy[3]);
                return Math.min(Math.min(segmentDistance(x,y,l,t,r,t),segmentDistance(x,y,r,t,r,b)),
                    Math.min(segmentDistance(x,y,r,b,l,b),segmentDistance(x,y,l,b,l,t)));
            }
            case CIRCLE:{float radius=(float)Math.hypot(xy[2]-xy[0],xy[3]-xy[1]);return Math.abs((float)Math.hypot(x-xy[0],y-xy[1])-radius);}
            case POLYLINE:{
                if(xy.length<4)return Float.POSITIVE_INFINITY;float best=Float.POSITIVE_INFINITY;
                for(int i=2;i+1<xy.length;i+=2)best=Math.min(best,segmentDistance(x,y,xy[i-2],xy[i-1],xy[i],xy[i+1]));
                if(closed&&xy.length>=6)best=Math.min(best,segmentDistance(x,y,xy[xy.length-2],xy[xy.length-1],xy[0],xy[1]));
                return best;
            }
    ...
    private static float segmentDistance(float px,float py,float ax,float ay,float bx,float by){
        float dx=bx-ax,dy=by-ay;float len=dx*dx+dy*dy;if(len<=1e-12f)return (float)Math.hypot(px-ax,py-ay);
        float t=((px-ax)*dx+(py-ay)*dy)/len;t=Math.max(0f,Math.min(1f,t));
        return (float)Math.hypot(px-(ax+t*dx),py-(ay+t*dy));
    }
    private static float normalize(float degrees){
        if(!Float.isFinite(degrees))return 0f;float v=degrees%360f;if(v<=-180f)v+=360f;if(v>180f)v-=360f;return v;
    }
}
