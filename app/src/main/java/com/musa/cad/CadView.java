...
public class CadView extends View {
    ...
    private boolean breakArmed,lastUndoWasRegular,stretchArmed;
    private int stretchVertex=-1;
    ...
    public boolean defineBlockFromSelected(String rawName){
        ...
        String name=CadBlock.normalizeName(rawName);if(name.isEmpty())return false;
        CadEdit member=CadBlock.memberRelativeToCenter(selected);if(member==null)return false;
        userBlocks.put(name.toUpperCase(Locale.ROOT),new CadBlock.Definition(name,Collections.singletonList(member)));
        notifyValue();invalidate();return true;
    }
    public boolean armInsertBlock(String rawName,float blockScale,float rotation){
    ...
    public boolean armStretchSelected(){
        CadEdit selected=sourceEdits.currentSelected();
        if(mode!=Mode.SELECT_ENTITY||selected==null||selected.stretchVertexCount()<1)return false;
        stretchArmed=true;stretchVertex=-1;pairCommand=PairCommand.NONE;breakArmed=false;moveSelectedArmed=false;notifyValue();invalidate();return true;
    }
    public boolean hatchSelected(String pattern,float angle,float patternScale){
        if(mode!=Mode.SELECT_ENTITY||!sourceEdits.hasSelection())return false;
        CadEdit selected=sourceEdits.currentSelected();float[] boundary=hatchBoundary(selected);if(boundary==null)return false;
        CadEdit hatch=CadEdit.hatch(boundary,pattern,angle,patternScale);if(hatch==null)return false;
        addRegularEdit(hatch);lastActionRegular=true;notifyValue();invalidate();return true;
    }
    private static float[] hatchBoundary(CadEdit e){
        if(e==null)return null;
        if(e.type==CadEdit.Type.RECTANGLE&&e.xy.length>=4){
            float x1=e.xy[0],y1=e.xy[1],x2=e.xy[2],y2=e.xy[3];return new float[]{x1,y1,x2,y1,x2,y2,x1,y2};
        }
        if(e.type==CadEdit.Type.POLYLINE&&e.closed&&e.xy.length>=6)return e.xy.clone();
        if(e.type==CadEdit.Type.CIRCLE&&e.xy.length>=4){
            float cx=e.xy[0],cy=e.xy[1],r=(float)Math.hypot(e.xy[2]-cx,e.xy[3]-cy);if(r<1e-6f)return null;float[] out=new float[96*2];
            for(int i=0;i<96;i++){double a=Math.PI*2d*i/96d;out[i*2]=cx+(float)Math.cos(a)*r;out[i*2+1]=cy+(float)Math.sin(a)*r;}return out;
        }
        if(e.type==CadEdit.Type.ELLIPSE&&e.xy.length>=6){
            float cx=e.xy[0],cy=e.xy[1],ax=e.xy[2]-cx,ay=e.xy[3]-cy,bx=e.xy[4]-cx,by=e.xy[5]-cy;float[] out=new float[96*2];
            for(int i=0;i<96;i++){double a=Math.PI*2d*i/96d,co=Math.cos(a),si=Math.sin(a);out[i*2]=cx+(float)(ax*co+bx*si);out[i*2+1]=cy+(float)(ay*co+by*si);}return out;
        }
        return null;
    }
    ...
    public boolean copySelectedEntity(){
        ...
        CadEdit copy=sourceEdits.copySelected(offset,-offset);if(copy==null)return false;addRegularEdit(copy);lastActionRegular=true;notifyValue();invalidate();return true;
    }
    public int arraySelectedEntity(int rows,int columns,float columnSpacing,float rowSpacing){
        ...
        for(int r=0;r<rows;r++){
            for(int c=0;c<columns;c++){
                ...
                if(copy!=null){addRegularEdit(copy);added++;}
            }
        }
    ...
    public boolean explodeSelectedEntity(){
        ...
        addRegularEdits(parts);lastActionRegular=true;moveSelectedArmed=false;notifyValue();invalidate();return true;
    }
    ...
    public boolean finishEdit(){
        ...
        addRegularEdit(CadEdit.polyline(xy));lastActionRegular=true;points.clear();lastSnapped=false;notifyValue();invalidate();return true;
    }
    ...
    private void drawEdit(Canvas c,CadEdit edit,int color){
        ...
        switch(edit.type){
            ...
            case HATCH:{drawHatch(c,edit,color);break;}
            case INSERT:{drawBlockInsert(c,edit,color);break;}
            case TEXT:{
    ...
    private void drawHatch(Canvas c,CadEdit edit,int color){
        float[] v=map(edit.xy);if(v.length<6)return;Path path=new Path();path.moveTo(v[0],v[1]);for(int i=2;i+1<v.length;i+=2)path.lineTo(v[i],v[i+1]);path.close();
        int oldAlpha=paint.getAlpha();Paint.Style oldStyle=paint.getStyle();RectF b=new RectF();path.computeBounds(b,true);
        if("SOLID".equalsIgnoreCase(edit.text)){paint.setStyle(Paint.Style.FILL);paint.setAlpha(72);c.drawPath(path,paint);paint.setAlpha(oldAlpha);paint.setStyle(Paint.Style.STROKE);c.drawPath(path,paint);}
        else{
            int save=c.save();c.clipPath(path);float cx=b.centerX(),cy=b.centerY(),diag=(float)Math.hypot(b.width(),b.height())*1.5f;float spacing=Math.max(7f,edit.textHeight*Math.max(.001f,scale));
            c.rotate(45f+edit.rotationDegrees,cx,cy);paint.setStyle(Paint.Style.STROKE);paint.setAlpha(160);
            for(float y=cy-diag;y<=cy+diag;y+=spacing)c.drawLine(cx-diag,y,cx+diag,y,paint);
            c.restoreToCount(save);paint.setAlpha(oldAlpha);paint.setStyle(Paint.Style.STROKE);c.drawPath(path,paint);
        }
        paint.setAlpha(oldAlpha);paint.setStyle(oldStyle);
    }

    private void drawBlockInsert(Canvas c,CadEdit insert,int color){
        if(insert.xy.length<2||insert.text==null)return;CadBlock.Definition def=userBlocks.get(insert.text.toUpperCase(Locale.ROOT));if(def==null)return;
        float scaleValue=insert.insertScale();
        for(CadEdit member:def.members){
            CadEdit placed=member.scaled(scaleValue,0f,0f).rotated(insert.rotationDegrees,0f,0f).translated(insert.xy[0],insert.xy[1]);
            drawEdit(c,placed,color);
        }
    }
    ...
    private static void addCloudSide(List<CadEdit> out,float x1,float y1,float x2,float y2,float nx,float ny){
        float length=(float)Math.hypot(x2-x1,y2-y1);if(length<1e-5f)return;
        int segments=Math.max(2,Math.min(10,(int)Math.ceil(length/Math.max(18f,length/6f))));
        for(int i=0;i<segments;i++){
            float t0=i/(float)segments,t1=(i+1)/(float)segments;
            float ax=x1+(x2-x1)*t0,ay=y1+(y2-y1)*t0,bx=x1+(x2-x1)*t1,by=y1+(y2-y1)*t1;
            float mx=(ax+bx)*.5f,my=(ay+by)*.5f,bulge=(float)Math.hypot(bx-ax,by-ay)*.32f;
            CadEdit arc=CadEdit.arc(ax,ay,mx+nx*bulge,my+ny*bulge,bx,by);if(arc!=null)out.add(arc);
        }
    }

    private void addAngularDimension(PointF a,PointF vertex,PointF c){
        float u1x=a.x-vertex.x,u1y=a.y-vertex.y,u2x=c.x-vertex.x,u2y=c.y-vertex.y;
        float l1=(float)Math.hypot(u1x,u1y),l2=(float)Math.hypot(u2x,u2y);
        if(l1<1e-6f||l2<1e-6f){if(listener!=null)listener.onMeasurement("Açısal ölçü • Geçerli 3 nokta seçin");return;}
        u1x/=l1;u1y/=l1;u2x/=l2;u2y/=l2;
        double dot=Math.max(-1d,Math.min(1d,u1x*u2x+u1y*u2y)),angle=Math.acos(dot);
        float radius=Math.max(dimTextHeightContent*3f,Math.min(l1,l2)*.62f);
        float sx=vertex.x+u1x*radius,sy=vertex.y+u1y*radius,ex=vertex.x+u2x*radius,ey=vertex.y+u2y*radius;
        float bx=u1x+u2x,by=u1y+u2y,bl=(float)Math.hypot(bx,by);
        if(bl<1e-5f){bx=-u1y;by=u1x;bl=1f;}bx/=bl;by/=bl;
        float mx=vertex.x+bx*radius,my=vertex.y+by*radius;
        CadEdit arc=CadEdit.arc(sx,sy,mx,my,ex,ey);
        if(arc==null){if(listener!=null)listener.onMeasurement("Açısal ölçü • Yay oluşturulamadı");return;}
        ArrayList<CadEdit> out=new ArrayList<>();
        out.add(CadEdit.line(vertex.x,vertex.y,a.x,a.y));out.add(CadEdit.line(vertex.x,vertex.y,c.x,c.y));out.add(arc);
        float dsx=mx-sx,dsy=my-sy,dsl=(float)Math.hypot(dsx,dsy);if(dsl>1e-6f)addRadialArrow(out,sx,sy,dsx/dsl,dsy/dsl,dimArrowSizeContent);
        float dex=mx-ex,dey=my-ey,del=(float)Math.hypot(dex,dey);if(del>1e-6f)addRadialArrow(out,ex,ey,dex/del,dey/del,dimArrowSizeContent);
        String label=CadDimension.format(Math.toDegrees(angle),dimPrecision)+"°";
        out.add(CadEdit.styledText(vertex.x+bx*(radius+dimTextHeightContent*.85f),vertex.y+by*(radius+dimTextHeightContent*.85f),label,0f,"STANDARD","sans",false,dimTextHeightContent,1f,0f,0));
        addRegularEdits(out);lastActionRegular=true;redoEdits.clear();
        if(listener!=null)listener.onMeasurement("Açısal ölçü: "+formatMeasured(Math.toDegrees(angle))+"°");
    }
    ...
    private void measureArcLengthAt(PointF point){
        ...
        float tolerance=22f*getResources().getDisplayMetrics().density/Math.max(.001f,scale);
        DxfParser.SourceEntity source=vectorDrawing.findEditableSource(point.x,point.y,tolerance,sourceEdits.hiddenSourceIds());
        if(source==null){listener.onMeasurement("Yay uzunluğu • Yay veya daire bulunamadı");return;}
        CadEdit edit=source.prototype();double contentLength;
        if(edit.type==CadEdit.Type.CIRCLE&&edit.xy.length>=4){
            double r=Math.hypot(edit.xy[2]-edit.xy[0],edit.xy[3]-edit.xy[1]);contentLength=2d*Math.PI*r;
        }else if(edit.type==CadEdit.Type.ARC&&edit.xy.length>=8){
            double cx=edit.xy[6],cy=edit.xy[7];
            double start=Math.atan2(edit.xy[1]-cy,edit.xy[0]-cx),mid=Math.atan2(edit.xy[3]-cy,edit.xy[2]-cx),end=Math.atan2(edit.xy[5]-cy,edit.xy[4]-cx);
            double se=positiveAngle(end-start),sm=positiveAngle(mid-start);double sweep=sm<=se+1e-8?se:(Math.PI*2d-se);
            double r=Math.hypot(edit.xy[0]-cx,edit.xy[1]-cy);contentLength=r*sweep;
        }else{listener.onMeasurement("Yay uzunluğu • Seçilen nesne yay veya daire değil");return;}
        listener.onMeasurement("Yay uzunluğu: "+formatMeasured(contentLength*unitsPerImagePixel)+" "+unitName);
    }
    private static double positiveAngle(double value){double full=Math.PI*2d;value%=full;if(value<0d)value+=full;return value;}
    ...
    private void applyStretch(float x,float y){
        CadEdit target=sourceEdits.currentSelected();
        if(target==null||target.stretchVertexCount()<1){stretchArmed=false;stretchVertex=-1;notifyValue();invalidate();return;}
        if(stretchVertex<0){
            float tolerance=30f*getResources().getDisplayMetrics().density/Math.max(.001f,scale),best=tolerance;int found=-1;
            for(int i=0;i<target.stretchVertexCount();i++){float vx=target.stretchVertexX(i),vy=target.stretchVertexY(i);float d=(float)Math.hypot(x-vx,y-vy);if(d<=best){best=d;found=i;}}
            if(found<0){if(listener!=null)listener.onMeasurement("STRETCH • Taşınacak köşe/vertex noktasına dokunun");return;}
            stretchVertex=found;performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP);notifyValue();invalidate();return;
        }
        CadEdit replacement=target.stretchedVertex(stretchVertex,x,y);
        if(replacement!=null&&sourceEdits.replaceSelected(replacement)){
            redoEdits.clear();lastActionRegular=false;lastUndoWasRegular=false;performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP);
            stretchArmed=false;stretchVertex=-1;invalidate();if(listener!=null)listener.onMeasurement("STRETCH • Vertex yeni konuma taşındı");return;
        }
        if(listener!=null)listener.onMeasurement("STRETCH • Seçili nesne bu noktadan esnetilemedi");
    }
    ...
    private void applyBreak(float x,float y){
        ...
        addRegularEdit(first);addRegularEdit(second);breakArmed=false;lastActionRegular=true;performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP);invalidate();
    ...
    private void applyPairCommand(float x,float y){
        ...
        if(target==null){pairCommand=PairCommand.NONE;notifyValue();invalidate();return;}
        if(command!=PairCommand.MATCHPROP&&command!=PairCommand.JOIN&&(target.type!=CadEdit.Type.LINE||target.xy.length<4)){pairCommand=PairCommand.NONE;notifyValue();invalidate();return;}
        ...
        addRegularEdit(connector);lastActionRegular=true;pairCommand=PairCommand.NONE;performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP);invalidate();
        if(listener!=null)listener.onMeasurement(pairCommandName(command)+" • İşlem tamamlandı");
    }
    ...
    private void commitFreehand(){
        ...
        float avg=freehandPressureSamples==0?.5f:freehandPressureSum/freehandPressureSamples;addRegularEdit(CadEdit.freehand(xy,pressureWidth(avg)));lastActionRegular=true;freehandPoints.clear();
    }
    ...
    private boolean addCadPoint(float screenX,float screenY){
        ...
        if(mode==Mode.DRAW_NUMBER){addRegularEdit(CadEdit.text(xy[0],xy[1],Integer.toString(numberingNext++)));lastActionRegular=true;lastSnapped=false;notifyValue();invalidate();return true;}
        ...
        else if(mode==Mode.DRAW_CIRCLE&&points.size()==2){PointF a=points.get(0),b=points.get(1);addRegularEdit(CadEdit.circle(a.x,a.y,b.x,b.y));lastActionRegular=true;points.clear();lastSnapped=false;}
        else if(mode==Mode.DRAW_ARROW&&points.size()==2){
            PointF tip=points.get(0),tail=points.get(1);float dx=tail.x-tip.x,dy=tail.y-tip.y,len=(float)Math.hypot(dx,dy);
            if(len>1e-6f){
                float ux=dx/len,uy=dy/len,nx=-uy,ny=ux;float size=Math.max(10f,Math.min(42f,len*.18f));
                ArrayList<CadEdit> arrow=new ArrayList<>();arrow.add(CadEdit.line(tip.x,tip.y,tail.x,tail.y));
                arrow.add(CadEdit.line(tip.x,tip.y,tip.x+ux*size+nx*size*.42f,tip.y+uy*size+ny*size*.42f));
                arrow.add(CadEdit.line(tip.x,tip.y,tip.x+ux*size-nx*size*.42f,tip.y+uy*size-ny*size*.42f));
                addRegularEdits(arrow);lastActionRegular=true;
            }
            points.clear();lastSnapped=false;
        }
        else if(mode==Mode.DRAW_MULTILEADER&&points.size()==2){
            PointF tip=points.get(0),tail=points.get(1);float dx=tail.x-tip.x,dy=tail.y-tip.y,len=(float)Math.hypot(dx,dy);
            if(len>1e-6f){
                float ux=dx/len,uy=dy/len,nx=-uy,ny=ux;float size=Math.max(10f,Math.min(42f,len*.18f));
                ArrayList<CadEdit> leader=new ArrayList<>();leader.add(CadEdit.line(tip.x,tip.y,tail.x,tail.y));
                leader.add(CadEdit.line(tip.x,tip.y,tip.x+ux*size+nx*size*.42f,tip.y+uy*size+ny*size*.42f));
                leader.add(CadEdit.line(tip.x,tip.y,tip.x+ux*size-nx*size*.42f,tip.y+uy*size-ny*size*.42f));
                float landing=Math.max(size*1.5f,dimTextHeightContent*2.2f);float dir=dx>=0f?1f:-1f;
                leader.add(CadEdit.line(tail.x,tail.y,tail.x+dir*landing,tail.y));
                addRegularEdits(leader);lastActionRegular=true;
                if(listener!=null)listener.onTextRequested(tail.x+dir*landing,tail.y-dimTextHeightContent*.25f);
            }
            points.clear();lastSnapped=false;
        }
        else if(mode==Mode.DRAW_REVCLOUD&&points.size()==2){
            PointF a=points.get(0),b=points.get(1);float left=Math.min(a.x,b.x),right=Math.max(a.x,b.x),top=Math.min(a.y,b.y),bottom=Math.max(a.y,b.y);
            ArrayList<CadEdit> cloud=new ArrayList<>();
            addCloudSide(cloud,left,top,right,top,0f,-1f);
            addCloudSide(cloud,right,top,right,bottom,1f,0f);
            addCloudSide(cloud,right,bottom,left,bottom,0f,1f);
            addCloudSide(cloud,left,bottom,left,top,-1f,0f);
            if(!cloud.isEmpty()){addRegularEdits(cloud);lastActionRegular=true;}
            points.clear();lastSnapped=false;
        }
        else if(mode==Mode.DRAW_ARC&&points.size()==3){PointF a=points.get(0),m=points.get(1),b=points.get(2);CadEdit arc=CadEdit.arc(a.x,a.y,m.x,m.y,b.x,b.y);if(arc==null){points.clear();lastSnapped=false;if(listener!=null)listener.onMeasurement("ARC • Üç nokta aynı doğru üzerinde olamaz");invalidate();return true;}addRegularEdit(arc);lastActionRegular=true;points.clear();lastSnapped=false;}
        else if(mode==Mode.DRAW_ELLIPSE&&points.size()==3){PointF c=points.get(0),a=points.get(1),b=points.get(2);CadEdit ellipse=CadEdit.ellipse(c.x,c.y,a.x,a.y,b.x,b.y);if(ellipse==null){points.clear();lastSnapped=false;if(listener!=null)listener.onMeasurement("ELLIPSE • Geçerli ana ve kısa eksen seçin");invalidate();return true;}addRegularEdit(ellipse);lastActionRegular=true;points.clear();lastSnapped=false;}
        ...
        else if(mode==Mode.DRAW_DIM_ANGULAR&&points.size()==3){addAngularDimension(points.get(0),points.get(1),points.get(2));points.clear();lastSnapped=false;}
    ...
    private void notifyValue(){
        ...
        if(mode==Mode.SELECT_ENTITY){
            ...
            if(stretchArmed){listener.onMeasurement(stretchVertex<0?"STRETCH • Taşınacak köşe/vertex noktasına dokunun":"STRETCH • Vertex için yeni konuma dokunun");return;}
        ...
        else if(mode==Mode.DRAW_DIM_ANGULAR)listener.onMeasurement("Açısal ölçü • İlk kol, köşe ve ikinci kol için 3 nokta seçin • Nokta: "+points.size());
        ...
        else if(mode==Mode.DRAW_NUMBER)listener.onMeasurement("Numbering • Yerleştirmek için dokunun • Sıradaki: "+numberingNext);
        else if(mode==Mode.DRAW_ARROW)listener.onMeasurement(points.isEmpty()?"Ok • Ok ucunu seçin":"Ok • Kuyruk noktasını seçin");
        else if(mode==Mode.DRAW_MULTILEADER)listener.onMeasurement(points.isEmpty()?"Multileader • Ok ucunu seçin":"Multileader • Metin bağlantı noktasını seçin");
        else if(mode==Mode.DRAW_REVCLOUD)listener.onMeasurement(points.isEmpty()?"Revcloud • İlk köşeyi seçin":"Revcloud • Karşı köşeyi seçin");
