...
public final class DxfWriter {
    public static void write(File baseDxf,OutputStream target,DxfParser.Result drawing,List<CadEdit> edits)throws IOException{
        write(baseDxf,target,drawing,edits,Collections.emptyList());
    }
    ...
    private static boolean isRemoved(int lineIndex,List<SourceRange> removals){
        for(SourceRange range:removals)if(range!=null&&range.containsLine(lineIndex))return true;return false;
    }
    ...
    private static void writeEdits(BufferedWriter out,Matrix contentToWorld,List<CadEdit> edits)throws IOException{
        ...
        for(CadEdit edit:edits){
            ...
            switch(edit.type){
                ...
                case POLYLINE:{entity(out,"LWPOLYLINE");layer(out);int count=edit.xy.length/2;i(out,90,count);i(out,70,edit.closed?1:0);for(int k=0;k+1<edit.xy.length;k+=2){PointF p=w(contentToWorld,edit.xy[k],edit.xy[k+1]);point(out,p.x,p.y);}break;}
                case TEXT:{PointF p=w(contentToWorld,edit.xy[0],edit.xy[1]);entity(out,"TEXT");layer(out);n(out,10,p.x);n(out,20,p.y);n(out,30,0);n(out,40,worldTextHeight(contentToWorld,30));n(out,50,-edit.rotationDegrees);tag(out,1,safe(edit.text));break;}
            }
        }
    }
