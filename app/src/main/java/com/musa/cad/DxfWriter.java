...
public final class DxfWriter {
    ...
    private static void writeReplacements(BufferedWriter out,Matrix contentToWorld,List<SourceReplacement> replacements,DxfParser.Result drawing)throws IOException{if(replacements==null)return;for(SourceReplacement replacement:replacements){if(replacement==null)continue;DxfParser.SourceEntity source=drawing.sourceById(replacement.sourceId);String layout=source==null?drawing.activeLayout:source.layout;Integer color=replacement.colorMode==SourceReplacement.COLOR_BYLAYER?null:replacement.colorMode==SourceReplacement.COLOR_BYBLOCK?Integer.MIN_VALUE:(replacement.color&0x00FFFFFF);writeEdit(out,contentToWorld,replacement.edit,replacement.layer,color,replacement.lineType,replacement.lineTypeScale,replacement.lineWeight,layout);}}
    ...
    private static void common(BufferedWriter o,String layer,Integer trueColor,String lineType,Double lineTypeScale,Integer lineWeight,String layout)throws IOException{tag(o,8,layer);if(layout!=null&&!layout.trim().isEmpty()&&!DxfBlocks.MODEL_LAYOUT.equalsIgnoreCase(layout.trim())){i(o,67,1);tag(o,410,layout.trim());}if(trueColor!=null){if(trueColor==Integer.MIN_VALUE)i(o,62,0);else tag(o,420,Integer.toString(trueColor));}if(lineType!=null&&!lineType.trim().isEmpty())tag(o,6,DxfLineStyle.normalizeName(lineType));if(lineTypeScale!=null&&Double.isFinite(lineTypeScale)&&lineTypeScale>0d&&Math.abs(lineTypeScale-1d)>1e-9)n(o,48,lineTypeScale);if(lineWeight!=null)i(o,370,DxfLineStyle.normalizeWeight(lineWeight,DxfLineStyle.DEFAULT_LINEWEIGHT));}
