...
public final class DxfWriter {
    ...
    public static void write(File baseDxf,OutputStream target,DxfParser.Result drawing,List<CadEdit> additions,List<SourceReplacement> replacements,List<SourceRange> removedSources)throws IOException{
        ...
        try(BufferedReader in=new BufferedReader(new InputStreamReader(new FileInputStream(baseDxf),cs),128*1024);BufferedWriter out=new BufferedWriter(new OutputStreamWriter(target,cs),128*1024)){
            while(true){FileTransfer.checkCancelled();String codeLine=in.readLine();if(codeLine==null)break;String valueLine=in.readLine();if(valueLine==null)throw new IOException("Eksik DXF etiketi");int code;try{code=Integer.parseInt(codeLine.trim());}catch(Exception e){throw new IOException("Geçersiz DXF etiketi",e);}String value=valueLine.trim();
                if(code==0&&"ENDSEC".equals(value)&&"ENTITIES".equals(section)&&!inserted){writeEdits(out,contentToWorld,additions,drawing.activeLayout);writeReplacements(out,contentToWorld,replacements,drawing);inserted=true;}
    ...
    private static void writeEdits(BufferedWriter out,Matrix contentToWorld,List<CadEdit> edits,String layout)throws IOException{
        ...
        for(CadEdit edit:edits){
            if(edit==null)continue;Integer aci=null,trueColor=null;
    ...
    private static void writeEdit(BufferedWriter out,Matrix contentToWorld,CadEdit edit,String sourceLayer,Integer aciColor,Integer trueColor,String lineType,Double lineTypeScale,Integer lineWeight,String layout)throws IOException{
        if(edit==null)return;FileTransfer.checkCancelled();String layerName=sourceLayer==null||sourceLayer.trim().isEmpty()?"0":sourceLayer;
        switch(edit.type){
            case LINE:{PointF a=w(contentToWorld,edit.xy[0],edit.xy[1]),b=w(contentToWorld,edit.xy[2],edit.xy[3]);entity(out,"LINE");common(out,layerName,aciColor,trueColor,lineType,lineTypeScale,lineWeight,layout);n(out,10,a.x);n(out,20,a.y);n(out,30,0);n(out,11,b.x);n(out,21,b.y);n(out,31,0);break;}
            case RECTANGLE:{PointF a=w(contentToWorld,edit.xy[0],edit.xy[1]),b=w(contentToWorld,edit.xy[2],edit.xy[3]);entity(out,"LWPOLYLINE");common(out,layerName,aciColor,trueColor,lineType,lineTypeScale,lineWeight,layout);i(out,90,4);i(out,70,1);point(out,a.x,a.y);point(out,b.x,a.y);point(out,b.x,b.y);point(out,a.x,b.y);break;}
            case CIRCLE:{PointF c=w(contentToWorld,edit.xy[0],edit.xy[1]),p=w(contentToWorld,edit.xy[2],edit.xy[3]);entity(out,"CIRCLE");common(out,layerName,aciColor,trueColor,lineType,lineTypeScale,lineWeight,layout);n(out,10,c.x);n(out,20,c.y);n(out,30,0);n(out,40,Math.hypot(p.x-c.x,p.y-c.y));break;}
            case POLYLINE:{entity(out,"LWPOLYLINE");common(out,layerName,aciColor,trueColor,lineType,lineTypeScale,lineWeight,layout);int count=edit.xy.length/2;i(out,90,count);i(out,70,edit.closed?1:0);for(int k=0;k+1<edit.xy.length;k+=2){PointF p=w(contentToWorld,edit.xy[k],edit.xy[k+1]);point(out,p.x,p.y);}break;}
            case TEXT:{PointF p=w(contentToWorld,edit.xy[0],edit.xy[1]);entity(out,"TEXT");common(out,layerName,aciColor,trueColor,lineType,lineTypeScale,lineWeight,layout);n(out,10,p.x);n(out,20,p.y);n(out,30,0);double h=worldTextHeight(contentToWorld,edit.hasTextStyle()?edit.textHeight:30f);n(out,40,h);n(out,50,-edit.rotationDegrees);if(edit.hasTextStyle()){tag(out,7,edit.textStyleName);if(Math.abs(edit.textWidthFactor-1f)>1e-6)n(out,41,edit.textWidthFactor);if(Math.abs(edit.textOblique)>1e-6)n(out,51,edit.textOblique);if(edit.textGenerationFlags!=0)i(out,71,edit.textGenerationFlags);}tag(out,1,safe(edit.text));break;}
        }
    }
